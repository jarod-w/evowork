...
> **更新于 2026-09-19（第 33 次）**。这份文件回答一个问题：**现在到哪了、下一步是什么、什么还不能信。**
...
| **P3.5-1** M10a 模型管理 | 🟢 | **密钥库**（`safeStorage` → `secrets.bin` 密文；两个明文文件一次性迁移后改名 `.migrated`；钥匙串不可用时显式二选一、**不静默写明文**）· **`app.toml` 的 `mode`**（拓扑权威，URL 反推退役为一次性兼容）· **模型表四层合并**（② > ②' > ③ > ①，被停用的模型留在列表里带原因）· **自定义模型**（`models.toml` + 必选协议适配类型 + 每条一把独立密钥槽）· **设置页**（六个分区，两个如实说没做）· `SecretInput`（01 §5.35）· 连通性检查 · 单任务预算与并发上限 · **2026-09-16：「模型」页按附件重做**（单卡形态 + 行内改/测/删 + 保存前测试连接 + 按厂商跳文档；其余几段条件显示，见 11 §4.4.1）· **2026-09-19：「测试连接」改为 `GET /models` 拉名单**（填完 Key 即可点，不要求先填模型名） | **真机上的钥匙串（U6）** —— 2026-09-08 已用真实 DeepSeek key 把整条链路跑通（见 §3），但那台机器没有 keyring，验到的是"不可用时的兜底分支"而不是 `safeStorage` 本身 |
