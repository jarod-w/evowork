...
> **更新于 2026-09-05（第 9 次）**。这份文件回答一个问题：**现在到哪了、下一步是什么、什么还不能信。**
...
| 测试 | **840 个，全部通过、零跳过** —— 办公运行时装上之后，原来靠 `skipIf` 跳过的两条改成**强制无扩展环境**，两条分支都验 |
...
| **P1-3** M2 前端 | 🟢 | token 层 · 基础组件 · 19 类 Item · 四类审批卡 · 三栏工作台 · 首页与 Composer · 任务列表与六组筛选 · 变更视图 · Electron 引导与 preload · 本机服务宿主 · **2026-09-06：应用外壳与侧边栏骨架（01 §3.1–3.3）· 线性图标集 · 补齐 §5.2/5.6/5.7 三个组件 · 六个 IPC 动作接线 · 回车→建任务→切页实测通过** | 语音输入 · `@` 候选真实数据源 · 虚拟滚动 · 模型选择器（缺 `model/list` 接入）· 回合失败原因的对话内渲染 |
...
| ✅ **回车能把任务跑起来**（M2 端到端） | **2026-09-06 实测**：真窗口里往输入框写值 + 按回车 → `thread/start` + `turn/start` → 任务进侧边栏、切到任务页、用户消息渲染出来。**过程中改出四个缺陷**，见下表。注意这条验的是**链路**，不是回答质量 —— 网关没起时回合会以「失败」结束，那是预期 |
...
**方法论**：这四条没有一条能被单元测试抓到，也没有一条能在命令行复现 —— 它们都要求
...
| ~~还没画的 UI~~ | ✅ **已完成**：资料库三栏 · 自动化表单与执行历史 · 用量与审计页 · 首运行六步引导（全部只用 01 §5 清单里的组件，**没有出现第 33 个**） | — |
