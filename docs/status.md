...
> **更新于 2026-09-12（第 22 次）**。这份文件回答一个问题：**现在到哪了、下一步是什么、什么还不能信。**
...
| 测试 | **1468 个通过、2 个跳过**（2026-09-13 实测 `vitest run --maxWorkers=1`，104 个文件）。跳过的是 runtime-installer 的两条联网端到端安装测试；办公技能的实际 docx / xlsx / pptx / png 生成分支本次均已运行并通过。**"没装扩展怎么办"那条不跳** —— 它由夹具强制构造（2026-09-06 修好，此前那个夹具名不副实，见 §3） |
...
| 还没画的 UI | ✅ **已完成**：资料库三栏 · 自动化列表与执行历史（表单组件写了但**没挂**，见 §6.1）· 用量与审计页 · 首运行六步引导 · 项目列表与详情 · 设置页（六分区）· **技能·连接器三 Tab**（专家空态诚实；办公技能随包；browser MCP 待信任）。组件清单现为 **35** 个（33 ProgressBar · 34 Dialog / ItemCard · 35 SecretInput，都先补进 01 §5 再实现），**2026-09-09 逐个核对过，35 个都有实现** | 剩：通知中心 · 任务页的排队区 · 结果区四视图（见 §6.1） |
...
| **02 信息架构** | 通知中心与设备中心（接通前已隐藏）· 独立搜索页与正文搜索 · deeplink `evowork://` | `app.tsx` · `sidebar.tsx` |
| **03 首页与 Composer** | `@` 候选 / `/` 命令 / 附件 / 排队追问 / 语音组件能力未由 `app.tsx` 传入 | `composer.tsx` vs `app.tsx` · `config/showcase/.gitkeep` |
| **04 任务工作台** | A · 系统主动打开结果区的意图判定 · 排队区 / 中断转向 / 子任务侧滑未端到端 | `item-renderers.tsx` · `app.tsx` · `task-workspace.tsx` |
