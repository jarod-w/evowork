...
> **更新于 2026-09-16（第 29 次）**。这份文件回答一个问题：**现在到哪了、下一步是什么、什么还不能信。**
...
>
> **第 29 次回写视觉真源**：C1 改为工作台基线（冷灰中性层、发丝分割、焦点蓝），C3–C6 产品行为与 C7（本版不交付暗色开关）保留；01 §2–§4 与相关页面规格已改。**代码仍是旧暖灰 token 与 260/800/900 断点**，在 token 包与桌面壳落地前不能把新视觉当成已接通。
...
| 测试 | **1514 个通过、2 个跳过**（2026-09-13 实测，106 个文件）。跳过的是 runtime-installer 的两条联网端到端安装测试；本轮新增队列启动/失败保留、附件发送、后台任务运行态和安全预览等回归。**"没装扩展怎么办"那条不跳** —— 它由夹具强制构造（2026-09-06 修好，此前那个夹具名不副实，见 §3） |
...
| **P2-1** M3 办公技能与解析 | 🟢 | **四个技能全部完成**（documents / spreadsheets / presentations / charts，共用一套骨架）· **本机解析管道**（识别 / 六道闸门 / 内置解析器 / zip / 注入载荷）· **三档运行时探测** · **办公扩展 App 内安装器**（2026-09-07，`services/runtime-installer`）· 附件选择已接 Composer 与宿主 | office / ocr 档的**实际解析器一个都没有**（`ingest/src/parsers/` 只有 `builtin.ts` 与 `zip.ts`；接口已定，等受限子进程）—— 所以 docx/pdf 当前会显示解析失败并允许改用原始文件引用 · OCR 档的安装（安装器只装 office 档，`pytesseract` 缺） |
...
| 需要接线 | ✅ **已接**：scheduler↔内核 · 自动化创建/编辑/运行↔权威表与调度器 · 产物 watcher（轮询 + 对账）↔产物索引 · Composer 附件↔本机 ingest · 队列↔回合完成后的显式 start · 四个技能↔办公运行时 · 安装器↔引导与探针 · hook 审计 JSONL↔`audit_log` 表 | **仍未接**：分享上传与云端托管端点；外部 office/OCR 解析器↔受限子进程；`wake_system`↔OS 唤醒事件 |
| 还没画的 UI | ✅ **已完成**：资料库三栏 · 自动化列表/历史/编辑 · 用量与审计页 · 首运行六步引导 · 项目列表与详情 · 设置页 · 插件三 Tab · 可见队列 · 结果区四视图。组件清单现为 **35** 个，且都有实现 | 剩：通知中心、设备中心，以及已明确列出的队列编辑/重排、搜索高级筛选等非主路径 |
...
| 篇 | 未做 / 半做 | 证据 |
| --- | --- | --- |
| **01 设计系统** | 文档已改走 ui-design 工作台数值；代码仍是暖灰 token、侧栏 260、内容列 800、最小窗 900 · 应用图标仍是 Electron 默认 | `packages/tokens/src/css.ts` · `sidebar.tsx` · `app.tsx` · `build/` |
...
| **08 产物与解析** | D · office / ocr **解析器文件不存在**（`parsers/` 只有 `builtin.ts` `zip.ts`）· OCR 档安装 · 分享页跟分享上传走（不在 M10b） | `services/ingest/src/parsers/` |
...
**仓内为空、但设计依赖的目录**：`ext/` · `config/showcase/` · `config/permissions/`（都只有 README 或 `.gitkeep`）。`plugins/agents/` 故意不预置角色。`apps/web/` 与 `services/identity/` 已有实现；分享页仍未做。
