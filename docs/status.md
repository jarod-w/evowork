...
| 测试 | **1474 个通过、2 个跳过**（2026-09-13 实测，105 个文件）。跳过的是 runtime-installer 的两条联网端到端安装测试；本轮新增队列启动/失败保留、附件发送、后台任务运行态和安全预览等回归。**"没装扩展怎么办"那条不跳** —— 它由夹具强制构造（2026-09-06 修好，此前那个夹具名不副实，见 §3） |
...
| # | 现象 | 证据 | 为什么要紧 |
| --- | --- | --- | --- |
| C | **侧栏旧任务行能力仍窄** | 侧栏只显示已接通的归档、删除；任务顶栏已接重命名、分叉、归档、删除。置顶、移动项目、打开文件夹仍无动作，因此继续不展示 | 现在没有“点了没反应”的假动作，但 04 §3.3 的完整行操作仍未覆盖 |
...
| **04 任务工作台** | C · 搜索高级筛选与单消息定位 · 队列编辑/重排 · 子任务侧滑未端到端 | `item-renderers.tsx` · `app.tsx` · `task-workspace.tsx` |
