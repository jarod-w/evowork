...
> ①c **多回合时序旅程 + 一个真缺陷**：运行中「默认排队不是插话」（04 §5.5 的产品决定）·
> 勾了立即插话后**那句话真的进了模型请求** · **Esc 停下回合**（键盘那条中断路径只有真窗口发得出来）。
> **缺陷**：「编辑排队项」用的是 `window.prompt`，而 **Electron 直接抛
> 「prompt() is not supported.」**—— 点 ✎ 什么都不会发生，界面上毫无提示。
> 渲染层里只有这一处 `prompt`，已改成应用内 Dialog（与任务重命名同一套做法）。
...
> 只会**少一条链路**，而断言写在别的文件里、不会指向启动代码。现在启动进
> `apps/desktop/test/e2e/harness/`（runner / boot / fake-gateway），驱动留在各自的 spec 里。
> 两个 runner 还摘掉了子进程环境里的 `ELECTRON_RUN_AS_NODE` —— VS Code 的终端会设它
> （它自己就是 Electron 应用），继承之后 Electron 以普通 Node 启动、入口第一行就炸，
> **看起来像代码坏了，其实是环境**。
>
> 两条断言（由 evowork-cb 会话加在 spec 里）：① **模型目录真的被内核用上了** ——
> 跑完读内核日志，`Unknown model evowork/…` 必须是 0。判别力实测过：同一条查询在
> 加目录之前那次运行上返回 **4**，之后是 **0**。协议里其实有更干净的信号
> （`ThreadTokenUsage.model_context_window`），但它到适配层就停了（渲染层没落点），
> 哪天 token 用量接到 UI 上就该换成它。② **重试记账断言从偶发红改成跟着现实走**：
> 原先写死"只能有一次成功"，实测出现过 1290 / 2580 / 3870 —— 都是整数倍，
> **成因是记忆提取**（这条 E2E 自己在 config 里开了 `generate_memories = true`）
> 会在回合之后再发一次模型请求，而它是异步的，落在读 goal 之前还是之后全看运气。
> 现在先等网关安静（超时**抛错**，不静默往下走），再按"这个任务实际成功几次"算。
>
> **仍然开着的一条**（两份 E2E 的 config 都有）：`default_permissions` / `approval_policy`
> 写在 `[permissions.evowork-workspace]` 表头之后，按 TOML 属于那张表而不是顶层 ——
> 也就是说 `approval_policy` 从没以顶层配置生效过。补它会让审批相关断言跟着变，
> 所以没混进重构，留给产品侧拍板。
>
> **第 68 次：内核终于知道每个模型的上下文有多大。** 第 67 次留的那条（内核把我们每个模型都当
> 未知型号、统一按兜底的 272k 触发压缩）已修：宿主在**内核启动之前**生成
> `$CODEX_HOME/model-catalog.json` 并把 `model_catalog_json` 设成根键（每次启动重写 —— 路径是
> 机器相关的，内容随用户增删模型变）。目录里除了 slug / 名字 / 上下文，**其余逐字复刻内核今天
> 已经在用的兜底值** —— 这样行为差分只有上下文一处。尤其 `input_modalities` 保持
> `["text","image"]`：按能力表收窄成 `["text"]` 会让内核在发给网关之前把图悄悄摘掉，
> 而 D2 要求的是显式拒绝。
>
> **第一版把所有任务都打挂了**（`thread/start` 全回 -32600）：内核对目录有一条自定义反序列化，
> **每条模型必须带 `base_instructions`**，缺了整份配置加载失败。填的是我们自己的产品身份底稿
> （F25 的同一份），不是内核那份带 Codex 的。这个坑正是坚持"落地前用真内核验一遍"的理由。
> 已钉成 F34（兜底 272k 是我们做这件事的理由）与 F35（底稿是硬要求）。
>
> **验过的**：真模型（Kimi K3）两轮回路全过，且内核日志里 `Unknown model evowork/kimi-k3`
> **消失了**。剩下 `gpt-5.6-luna / terra` 两条仍是未知 —— 目录是整份替换，而那两个只在
> `codex-rs/tui/` 里用，我们碰不到。
>
> **第 67 次：四个「还会踩到的坑」逐个落地。** ① **e2e 偶发失败**修掉：断言按“第一个请求”认领
> 回合，而内核一次会话里不止发我们这一个请求（prewarm / 记忆提取都可能先到）——改成**按内容认领**。
> ② **重试 × token 预算**：实测 `used=1290 once=1290` —— **不重复计**，但失败那几次**一个 token
> 都没记上**。后者修不了：用量只在最后一帧里，流断了就没有那一帧，编一个比不记更糟。已记进 U 表。
> ③ **合盖睡一觉，定时任务会无声无息地丢**：`setInterval` 在系统睡眠期间不跑、醒来只触发一拍，
> 而按点判断算的是“从现在起的下一次”——睡过去的那次既没有 MISSED 记录也没有补跑，
> 因为 misfire 补偿**只在启动时跑过一次**（只有重启 App 才补得上）。现在两拍之间隔得太久就当成
> 一次重新开机，走同一套补偿逻辑；测试用“把判据关掉就一条记录都没有”证伪过。
> **这比 U3 说的「关机一夜」常见得多** —— 关机会重启 App，合盖不会。
> ④ **上下文超限的文案**：内核那句是英文且带 **Codex** 品牌（K5），而它由内核自己生成、网关改不掉。
> 改成按结构化的 `codexErrorInfo` 换我们自己的中文，不匹配英文原文（上游改个词就会静默失效）。
>
> **⑤ 一条确认存在、但这轮没修的**：内核把我们**每一个模型都当未知型号**（你机器上的日志：
> `Unknown model deepseek/deepseek-flash is used. This will use fallback model metadata.`），
> 于是统一按 `context_window: 272_000` 的 95% 触发压缩 —— 而 **GLM 只有 128k**：
> 长任务必然先撞厂商硬上限，压缩永远等不到；DeepSeek 反过来浪费掉 1M 里的七成多。
> 正规修法是给内核一份 `model_catalog_json`（按模型给真实上下文），但那会**整份替换**内核的
> 模型目录，且 `ModelInfo` 必填字段多、上游加一个字段就会让内核拒绝加载整份配置 —— 需要先决定。
>
> **第 66 次：把"从没合起来跑过"的两条链路跑了。** ① **两张真正会弹的审批卡**在真内核上验了：
> 命令审批（卡片带着命令本身，点允许之后**命令真的执行**）、文件改动审批（**清单不再是空的**、
> 工作区外的改动被标出来、点拒绝文件**一个字节都没落盘**）。触发文件改动卡时第一次探测写到了
> e2e 的家目录，内核没问——因为那在系统临时目录里，`:workspace` 本来就允许；换成仓库根才真的越权。
> ② **整条智能体回路用真模型跑通了**（`scripts/verify-agent-loop.mjs`，要 key、不进 check）：
> Kimi K3 与 DeepSeek Flash 各两轮，断言"文件真的落盘"**且"模型把读回来的内容说出来"**——
> 后者才证明工具结果真的进了模型的上下文（`tool_call_id` 对不上时它会反复调同一个工具，不报错）。
> 第二轮在同一任务里，专门撞思维链回传。**两条我原本判定为高风险的坑（工具结果回传、思维链回传）
> 因此被证伪**：现在的实现是对的。U 表新增并关闭 U8。
>
> **第 65 次：托管转发改成真流式 + 两条审批链路的真相。** ① 云端 identity 的 `/v1/responses`
> 原先是 `await upstream.arrayBuffer()` —— 整条响应读完才写回第一个字节，**托管模型因此完全没有流式**，
> 而下游内核 300 秒收不到帧就判整回合失败（答案越长越容易失败）。现在边收边转，用量从流里那条
> `response.completed` 取（口径不变），等响应头也有了上限。测试用「换回读完再转就超时」证伪过。
> ② 顺手收掉心跳的一个分叉：探测在途时内核退出，那一拍会重排出第二条心跳链，重启后变成双倍频率。
> **③ 一条要更正的话**：上一轮报的「JSON-RPC 没有请求超时 → 永远挂着」**不成立** ——
> 心跳（30s 一探、10s 超时、连败 3 次杀进程）本来就是那个上限，杀完 `resetPending` 会把
> in-flight 请求全部拒掉。`requestTimeoutMs` 默认不设是**刻意的**，因为 `turn/start`
> 在长任务里可以跑很久（那条注释早就写着）。
> **④ 队友那 13 处协议修复**在真内核上验了：停止、立即插话通过；**追问与权限审批两条链路
> 在产品当前配置下根本发不起来** —— `request_user_input` 只在 Plan 模式可用
> （我们三档模式全是 default），`request_permissions` 要一个 UnderDevelopment 的开关。
> e2e 把"发不起来"钉成了断言：哪天它们能发起来，那条测试会红并提示去验那两张卡。
>
> **第 64 次：上游断了要自动重试，并且把重试说出来。** 举一反三扫出来的同类缺陷，一次收掉五条：
> ① 网关原来把「连不上 / 流中途断 / 厂商 5xx」映射成 `server_is_overloaded`，而它在内核里
> **是终止态、且会丢掉 message**（F33）—— 最该重试的三种故障一次都没重试过，用户看到的还是
> 「模型满了，换一个吧」。现在三处都改成内核会退避重试的语义（F32），看门狗那条也一并改成可重试。
> ② 内核重试期间会发 `error` + `willRetry: true`，而适配层**根本没订阅这条通知** ——
> 那段等待在界面上完全是空白。现在时间线上是一行「上游断了，正在尝试重新连接（2/5）」，
> 下一条内容一到就撤掉（内核不会专门说"我重连上了"）。
> ③ 重试次数从内核默认的 5 降到 **2**：一次重试要重发整段上下文，而"上游卡死"那种还要先等满
> 120 秒看门狗。模板 + **已有安装的迁移**都写了（只改模板修不了已装用户，F26 的教训）。
> ④ 设置页的「检查」原来只看 HTTP 200，而网关把模型层面的失败放在**流里** ——
> 密钥错也会被回答成"通了"。现在读到第一帧有结论的事件为止（`created` 和心跳都不算结论）。
> ⑤ `PORT` 拼错时 `listen()` 同步抛，日志里什么都不剩；网关与账号服务都改用容错解析。
> **验过的**：全仓 1883 项 + 协议形状自检 + 漂移雷达（新增 F32 / F33，BROKEN 0），以及
> **真实 `codex-app-server` 的端到端**：假网关回一条 `response.failed`，内核真的退避重试、
> 界面真的收到「正在尝试重新连接」、重试用完后**网关写的那句中文真的显示出来了**。
> 同一条测试还纠正了一个读代码读错的数：**`stream_max_retries = 2` 不等于"上游被打 2 次"** ——
> 实测 5～6 次（默认 5 时是 11 次），因为内核在重试用完后还会切一次传输通道并把计数清零
> （`core/src/responses_retry.rs:95-111`）。模板注释与 e2e 断言都按实测值写。
> **没验过的**：真实厂商断流时的观感（假网关能造出"立刻失败"，造不出"断在一半"）。
>
> **第 63 次补上转发路径的看门狗（第 62 次自己带出来的洞）。** 心跳让内核不再 300 秒判死，
> 但托管转发那条路**只有心跳没有看门狗** —— 云端停在半路时本机网关会一直跳心跳，回合
> **永远转圈**（比原来的"五分钟后报错"更糟）。现在两条上游共用 `services/gateway/src/idle.ts`
> 的一套判据：转发的响应体空档超时会自己收尾（`response.failed` + 中文原因 + `[DONE]`），
> 等云端响应头也有上限（那一段没有心跳护着，而内核对"等头"没有任何超时）。
> 测试用「把看门狗关掉就超时」证伪过。
>
> **第 62 次修复「这一回合失败了 / idle timeout waiting for SSE」。** 真机上 kimi-k3 的回合以这句英文告终：
> 内核读 SSE 的循环 **300 秒收不到任何一帧就判整个回合失败**（F31），而网关有好几段"活着但一帧都不发"
> 的时间 —— 上游在想、思维链被能力表挡掉、工具调用参数要攒完整才发得出 `output_item.done`。
> 现在两件事：① 网关**每 15 秒补一帧 `response.in_progress`**（F30：内核显式忽略它，到不了前端，
> 但让空闲计时器重来），响应头也由它在上游回第一片之前就写出去；② 网关**自己先一步判上游死掉** ——
> 两片之间超过 120 秒（首片 300 秒，量级不同所以分开）就掐断上游并给一句中文原因。
> 失败码用 `invalid_prompt` 而不是 `server_is_overloaded`：后者在内核里是**终止且丢掉 message**，
> 界面上会变成"模型满了，换一个吧"。两个旋钮可用 `GATEWAY_SSE_HEARTBEAT_MS` / `GATEWAY_UPSTREAM_IDLE_MS` 调。
> **验过的**：网关 157 项 + 全仓 `pnpm run check`；心跳与看门狗各有真 socket / 真管道的测试；
> **心跳帧本身在真实 `codex-app-server` 上实测过**（桌面 E2E 的假网关现在每条响应都夹带三帧心跳：
> 回合照常完成，且时间线上不出现它 —— 这条就是 F30 的实测面，以后上游若改掉忽略清单会直接红）。
> **没验过的**：真实上游卡死时的端到端表现（要一个会半死的 endpoint 才能造），以及 15 秒这个间隔在
> 长回合里的实际开销 —— 量级是每 15 秒几十字节，但没有实测。
>
> **第 61 次修复记忆 V1/V2 状态错配。** 上游缺省记忆管线是 V1，而
> `memory/status` 只统计 V2；此前 V1 已正常注入时，设置页仍可能长期显示“正在学习 / 0 个任务”并
> 持续轮询。现在适配层从有效配置读取 `memories.version`：V2 才请求和轮询进度，
> V1 明确说明当前协议不提供整理进度，但使用与生成设置仍有效；不强行切换用户的管线版本。
>
> **第 60 次完成记忆 P1 收口。** 实验方法被上游移除时，设置页会明确禁用
> 「清空全部」并说明降级，Composer 退回全局记忆设置；V2 记忆尚未就绪时，个性化页会自动刷新
> `memory/status`。真实 app-server + Electron E2E 新增内核 fixture，并在模型网关请求上断言记忆标记已注入；
> 这条证明“使用已有记忆”真正生效，而不只是配置可写。生产代码仍不读内核生成文件。
>
...
> 连续通知还会出现重复卡片。现在该通知已进入明确订阅面；其他未知协议通知仍只记不含正文的
> 形状用于漂移诊断，不再进入对话区。真正未知的 `ThreadItem` 仍保留“新类型事件”兜底。
> 完整仓库检查 1790 项通过、2 项条件跳过。
>
> **第 57 次完成技能/@ 剩余高优先级收口。** 连接器详情改读 app-server 实时 MCP 状态与
> 工具清单，支持 OAuth 授权、重载和逐工具“默认/每次确认/直接允许”；授权 URL 只由主进程打开。
> Composer 补全使用 combobox/listbox 语义、方向键读屏高亮与前缀优先稳定排序。新增真实隐藏
> Electron + 真实 `codex-app-server` E2E，覆盖 `$skill-creator` 精确路径、运行中排队与编辑、
> 历史恢复、杀进程后的自动重连和技能根重注册；同时修复 `turn/started` 到状态通知之间的排队
> 竞态，并按当前 queue extension 的 idle 自动出队语义移除重复 `thread/queue/start`。公开远程
> marketplace 仍未开放；建议先做可被企业关闭的只读浏览与安装前来源/版本/权限摘要，待产品确认。
>
> **第 56 次完成多代理上下文与对话语义对齐。** 首装模板和已有安装迁移都显式启用
> `multi_agent_v2`，固定根代理 + 3 个子代理、`wait_agent` 可用，并取消只限非代码模式；同轮修复
> 早期模板把根级 `default_permissions` / `approval_policy` 误写进权限子表、导致内核整份配置回退
> 默认值的问题。子代理时间线改为只读，任何后续输入沿完整父链回到根任务，再由根代理按状态选择
> `send_message` 或 `followup_task`；停止、附件、队列、模型与审批档也归根任务。协作条目不再丢弃
> canonical path、sender、全部 recipients、动作、状态、消息摘要和 agent states。UI 明示“共享工作区，
> 但上下文只在 `fork_turns` 创建时复制，之后靠显式消息”。真实 `codex-app-server config/read` 已确认
> 四个 V2 配置值与根级权限配置生效；多代理漂移断言 F26–F29、配置迁移、嵌套父链路由、运行中/
> 空闲路由选择、消息保真、只读 UI 和重启恢复均有自动测试。尚未用真实付费模型跑一轮代理协作，
> 因而模型是否稳定选择正确工具属于模型质量验收，不是协议/桌面链路缺口。
>
> **第 55 次对齐 ChatGPT/Codex 的技能引用与 @ 体验。** 桌面宿主在握手与内核重启后注册
> 随包技能根，Composer 以 `skills/list` 为唯一技能真源，发送内部名与精确 `SKILL.md` 路径；
> `$` 显式选技能，`@` 统一发现文件/资料/技能，文件候选改走 app-server 模糊搜索并取消过期请求。
> 可见 token 与结构化引用同步删除，排队编辑保留引用；MCP 插件选择发送 `mcp://` 精确 mention。
> `skills/changed` 会刷新清单，加载错误、作用域和启停状态已进入 UI，并新增随包 `skill-creator`。
> 套件页已接 `plugin/list/install/uninstall`，当前刻意只查询本机与工作区市场，**未开放公开远程市场**。
> 协议、适配层、Composer 与桌面接缝已有自动测试；技能引用的真实内核/真窗口 E2E 已完成。
>
> **第 54 次：电脑操控继续收口，仍不能启用。** 新增引导技能、留存内容主动查看与删除边界提示；短路径 Unix socket 适配长用户目录。完整仓库检查 1703 项通过、2 项条件跳过。随后 macOS arm64 Helper 原生 release 编译及 `.app` 装配通过，并补纯策略测试与文本扩展选择代码；AX/TCC 真机、签名与发布验收仍未完成，剩余项见 [设计 12 §17](design/12-computer-use.md#17-未完成清单与验收条件2026-09-24)。
...
>
> **第 40 次移除已下架模型默认值**：`deepseek-v4-flash` 已从运行时模型目录、三个
> 场景包、内置场景副本、首装内核配置和企业白名单模板移除。交互任务从实时模型目录
> 解析后显式传模型；没有可用模型时在创建 thread 前拒绝。旧安装只迁移精确匹配该旧
> 默认值的顶层配置，不覆盖其他用户或企业设置。自动化现在固定保存所选模型，每次执行前
> 检查实时目录；模型下架或旧自动化没选模型时会暂停并提示重新选择，不会静默换档。
...
> **尚未做新一轮真窗口 E2E**。
>
> **第 37 次修复旁聊产生幽灵任务**：`thread/fork(ephemeral:true)` 返回的临时 thread
> 不能 `thread/read` / `thread/delete`，也不会进入 `thread/list`；此前事件路由却把它写进
> 持久投影，导致打开旁聊与删除该行都报 `-32600`，同时空的临时快照把内置场景偏好
> `evowork/deepseek-v4-flash` 显示成“用户选过但不可用”。现在旁聊只保存在当前进程内，
> 可见历史继承当前已加载时间线，权威刷新跳过它，关闭/删除只清理临时态；普通分叉和
> 持久任务删除仍走内核协议。事件、适配层、IPC 与 App 回归测试已覆盖；
> **尚未做新一轮真窗口 E2E**。
>
> **第 38 次修复新建任务残留旧任务错误**：历史读取失败、发送失败和任务模型回退
> 现在标记为任务级提示；点击侧栏「新建任务」、快捷键、新建命令、搜索面板新建及
> 「在此项目新建任务」都会统一清掉这些提示和旧任务的模型覆盖，回到当前可用的默认模型。
> 网关降级等全局提示仍保留。App 回归测试覆盖截图中的 `thread/read -32600` 与
> `evowork/deepseek-v4-flash` 同时残留场景；**尚未做新一轮真窗口 E2E**。
...
| 测试 | **1790 个通过、2 个跳过**（2026-09-25 实测，113 个测试文件通过，另 1 个文件因联网 e2e 条件跳过）。跳过的是 runtime-installer 的两条联网端到端安装测试。**"没装扩展怎么办"那条不跳** —— 它由夹具强制构造（2026-09-06 修好，此前那个夹具名不副实，见 §3） |
| 源码 | 约 35.8k 行（不含测试）+ 20.0k 行测试 |
| 内核补丁 | **0 个文件 / 0 行**（预算 5 / 500）—— 设计判定只剩 P4 品牌字符串一项待落 |
| 内核基线 | 断言基线 `d583e73c4d12`（2026-09-24）。本轮已重新核对 Goal（`tokenBudget` + 6 状态）、Queue update/reorder、searchOccurrences、`thread/goal/cleared` 与 Multi-agent V2；F3 检测器也已跟随上游实现重写，不再把“仍只有 Plan + Default”误报为断言破坏。**F1–F29 已全部纳入机器断言**，支持锚点局部范围与仓内接线检查；当前 `BROKEN 0`。 |
...
| **P1-1** M1 网关 | 🟢 | Responses↔Chat 全量翻译 · 三家 provider · 错误码映射 · 用量规范化 · SSE **+ 心跳与上游看门狗（F30/F31）** · 能力端点 · Q14 不落盘 · **三家真实 endpoint 实测** | 企业私有部署包 · `maxContextTokens` 实测 · 长压测 · **真实上游卡死的端到端表现** |
| **P1-2** M2a 服务层 | 🟢 | 协议层 · 16 张本机表 + 两个迁移器 · 适配层（会话/心跳/重启/恢复/降级/事件流/审批/场景/搜索/队列）· automation 与产物写入方 | — |
| **P1-3** M2 前端 | 🟢 | token 层 · **35 个组件全部有实现**（2026-09-09 逐个核对 01 §5.1–5.35）· 19 类 Item · 四类审批卡 · 三栏工作台 · 首页与 Composer · 任务列表与六组筛选 · 变更视图组件 · Electron 引导与 preload · 本机服务宿主 · **2026-09-06：应用外壳与侧边栏骨架（01 §3.1–3.3）· 线性图标集 · IPC 动作接线（清单与实现逐项相等由测试钉住）· 回车→建任务→切页实测通过 · 模型选择器端到端接通** · **2026-09-07：`openTask` 拉历史 · 项目页落地 · 「助理」入口整个下架**（方案保留在 02 §4.2）· **2026-09-08：设置页六分区** · **2026-09-09：现有“技能·连接器”三 Tab + “发现应用”抽屉 + 官方 browser MCP** · **2026-09-12：Approved UI 阶段 A–D 主体**（克制首页、侧栏、GFM 时间线、结果区、项目四 Tab、资料库、响应式与 a11y）· **2026-09-13：UI 方案主路径收口**（结构化引用与附件、运行中队列/插话、任务与消息动作、独立搜索、文本/图片/PDF/HTML 预览、自动化管理闭环）· **2026-09-25：任务流 P1 收口**（区域批注、Goal 进度/耗尽动作、子任务抽屉、日期/状态/项目高级搜索）· **2026-09-25：技能引用对齐**（`$`/`@`、内核模糊文件搜索、精确 Skill/MCP 引用、动态刷新与套件生命周期）· **2026-09-25：真实 Electron/app-server 技能引用、排队编辑与内核重启 E2E** | 语音 · deeplink · 套件安装真实内核 E2E |
...
| **P2-2** M4 安全与策略 | 🟢 | 三级路径策略（硬拦截对完全访问也生效）· 权限 profile 文案与平台限制 · 命令风险四维判定 · 并发与预算闸门 · guardian 映射 · 审计记录与链式哈希 · **hooks 策略包**（四个事件，决策可测）· **2026-09-06：审计链路接通**（hook → `audit.jsonl` → `audit_log` 表 → 审计页 + 导出，从「更多」可达）· **2026-09-09：签名策略包下发（R11 / M10c）** · **2026-09-21：Q45 审批三档已接线**（Composer → `send(modeId)` → `turn/start` 三字段；`auto_review` 仅单测）· **2026-09-25：Goal/预算进度与耗尽双动作端到端接通** | 沙箱层的实际接线（seatbelt/landlock 由内核提供，需在 turn/start 上验证）· 设置页「安全与权限」分区仍没有 10 §7 的本机安全能力页（现在只展示策略包状态）· **`auto_review` 未对着真实内核验证** · 内部只读路径的 Ask `ToolContributor`（`ext/` 还是空的，不再挡 Composer）· **Windows 隔离强度结论（见 U5）** |
...
| `scripts/` | 漂移雷达（F1–F25 机器复核）· 补丁预算 · 许可清单 · **provider 实测探针** | — |
...
4. ~~P3-3 M8~~ **核心与任务结果预览已完成**（2026-09-25）。mermaid、资料库三栏、时间线产物卡、文本/源码/Markdown/Office/图片/PDF/HTML 本机只读预览、文本选区批注及图片/PDF 区域坐标批注均已落地；`FileChange` 路径可点击，并在以任务真实 `cwd` 为边界、复核软链后进入应用内预览或 diff。剩分享的宿主接线与云端端点，以及资料库的资料树 / 删除 / 分享数据接线。
...
| 还没画的 UI | ✅ **已完成**：资料库三栏 · 自动化列表/历史/编辑 · 用量与审计页 · 首运行四步引导 · 项目列表与详情 · 设置页 · 插件三 Tab · 可见队列 · 结果区四视图 · 子任务侧滑 · 高级搜索。组件清单现为 **35** 个，且都有实现 | 剩：通知中心、设备中心 |
...
| **08 产物与解析** | D · OCR 档解析器与安装 · 分享页跟分享上传走 | `services/ingest/src/parsers/` · `file-preview.tsx` |
| **10 安全与权限 UX** | **`auto_review` 未对着真实内核验证** · §6 云端审计摘要 · §7 本机安全能力页 · Computer Use 真实 AX/截图留存与删除闭环未验（CU-R8） · 内部只读路径的 Ask `ToolContributor`（`ext/` 空） | `settings.tsx` · `ext/` · `composer.tsx` |
...
### 6.2 任务流后续队列（2026-09-25）

P0 内核协议重新对齐和 P1 任务流（含 2026-09-25 的四项收尾）已经完成。下列保留在后续队列：

1. **P2 工程工作流**：原生代码评审、行内评论、worktree/分支创建与 PR 链路。
2. **P2 浏览器与系统集成**：真实应用内浏览器（不只是 sandbox HTML）、通知中心、deeplink、分享/上传、Remote/多设备。
3. **P3 扩展能力**：OCR 扫描件、语音输入、资料库正文 FTS/团队空间、OS 唤醒自动化。
...
2026-09-24 原生编译追验：macOS 27.0 / CLT 27.0 上，Swift `6.4.0.34.1` 使用 macOS 27.0 SDK 完成 release 编译。先前的 SDK 不匹配提示是受限模块缓存的连带错误；修复窗口 bounds 转换的源码类型错误后，`node scripts/build-computer-use.mjs` 装配出 arm64 `.app`（Helper 约 211 KiB，plist 校验通过）。尚无原生自动测试、TCC/真实读写链路测试及正式签名公证；`releaseVerified=false` 不变。

2026-09-24 原生策略续做：抽出窗口唯一绑定与 UTF-16 文本范围纯逻辑，新增可重复运行的 Swift 原生策略测试（12 个检查点），并实现 `select_text` 的 `extend` 选区合并代码。release Helper 再次编译装配通过；真实 Helper 进程的长度帧 `health` 握手通过。全仓格式、lint、类型检查、1703 项测试与补丁预算复验通过，2 项联网安装 e2e 条件跳过。原生测试不涉及真实 AX 权限、目标 App 或截图；正式验收和 `releaseVerified=false` 边界不变。

剩余工作按 [设计 12 §17](design/12-computer-use.md#17-未完成清单与验收条件2026-09-24) 的 CU-R1–CU-R11 跟踪；原生测试、双向认证、完整动作和存储删除为优先前置，所有发布阻塞项验收前保持关闭。

## 2026-09-26 · 模型能力位收敛成一张表（读图缺陷）

**现象**：用户在设置页加了 `deepseek/deepseek-flash` 与 `moonshot/kimi-k3`（自有密钥），
Composer 的模型下拉里两条都把「读图」画成灰色划除，发图会被网关按
`IMAGE_INPUT_UNSUPPORTED` 拒掉。而**这两家的视觉能力都有依据**：kimi-k3 是 2026-09-05
我们自己对着真实 endpoint 测出来的（32×32 纯红图答"红色"），deepseek-flash 写在厂商
视觉指南里。

**根因不是"漏标了两个 true"**，是能力位当时有三条互不相通的来路，两条写死了 false：

| 层 | 原来的来路 | 同一个 kimi-k3 |
| --- | --- | --- |
| ① 内置 | `P0_MODELS` 手写（来自实测） | 会读图 |
| ②' 租户 | `parseTenantModels` 里的常量 | **不会读图** |
| ③ 自定义 | `DEFAULT_CUSTOM_CAPABILITIES` | **不会读图** |

保守默认本身没错（对陌生 endpoint 成立），错的是它盖过了我们**确实知道**的事实 ——
CLAUDE.md §9.1「两个模块各自对，合起来不对」。放大它的是设置页那个对话框：
它**根本没有能力位的输入项**（11 §4.4 只问供应商 / 密钥 / 模型名 / endpoint），
所以每一条自定义模型存下来的都是那个默认值。

**改法**：新增 [`services/gateway/src/known-models.ts`](../services/gateway/src/known-models.ts) ——
按 (协议适配类型, 上游真实模型名) 索引的一张表，三层共用，`P0_MODELS` 从它派生。
认得出来的按表走，认不出来的才落保守默认；`private` 一律认不出来。
读 `models.toml` 时按表重算，所以**用户机器上已经存坏的那两条自己会好**，不用删了重加。

**顺手修掉的第二个**：`updateCustomModel` 原先 `...target` 把能力位一路带过来 ——
把 `deepseek-v4-flash` 改成 `deepseek-flash`（升级型号是常规动作）之后，
新型号能读图、界面上仍划着「读图」。现在改型号就重算。

**验过什么**：`pnpm run check` 全绿（1834 项）。新增 12 + 3 + 5 + 5 项断言，
守的是"同一个型号不该因为走哪一层而失忆"与"认不出来时不许乱猜"。

**当天稍后拿到两把 key，实测收口（见下一节）。**

## 2026-09-26（同日）· 两家实测：读图成立，并订正一条 cache 口径

拿到 DeepSeek 与 Kimi 的 key，用 `scripts/verify-provider.mjs` 各跑一遍，**各 23 条全通过**。

**最要紧的一条：两家都真的能看图。** 判据不是 HTTP 200 —— 发一张 32×32 纯红 PNG 问颜色，
`deepseek-flash` 与 `kimi-k3` 都答出了"红"。这条必须这么验，因为 `imageInput` 有三种结局
（D2），而第三种"接受但看不见"回的就是 200：同厂的 `deepseek-v4-flash` 实测正是收下图、
回 200、然后说"无法识别"。**只看状态码的探针会把它判成支持。**

| | `deepseek-flash` | `kimi-k3` |
|---|---|---|
| 读图 | ✅ 答"红" | ✅ 答"红" |
| 推理 | ✅ 66 帧里 65 帧 | ✅ 64 帧里 61 帧 |
| 并行工具 | ✅ 一轮两个 tool_call | ✅ 一轮两个 tool_call |
| cache 命中 | 顶层 896 + 嵌套 896 | 顶层 1024 + 嵌套 1024 |
| 未知模型 | 400 + `code=invalid_request_error` | 404 + **只有 `type`** |

`deepseek-flash` 因此从 `evidence: 'vendor-doc'` 升为 `probe`（2026-09-26），**U7 关闭**；
`unverified` 只剩 `maxContextTokens`（要塞满上下文才能测）。

**补了两节探针**，它们是这一轮才发现没人验过的：

- **③b 并行工具调用** —— `parallelToolCalls` 是能力表里唯一一项三家都写着 true、
  却从来没被测过的能力位。原 ③ 只给一个工具，拿到一个 tool_call 什么也证明不了。
- **⑥ cache 命中时的字段名** —— ①② 只证明了"usage 里有 cache 字段"，而那是**未命中**的形状。
  判据只能是同一段长 prompt 发两次。

**订正一条口径**（不影响当前行为，但会误导下一个人）：`usage.ts` 原注释写成"三家给了三种写法"，
像是一家一条。实测 DeepSeek 与 Kimi 命中时**同时**给顶层 `cached_tokens` 与嵌套的
`prompt_tokens_details.cached_tokens`，两处数值一致。照"一家一条"去改成按厂商只读一条，
就会在另一家上漏读 —— 而漏读的表现是命中永远显示 0，**而 0 是"不支持 cache"的合法取值**。

**另一个顺手钉住的**：Kimi 未命中时给的是 `prompt_tokens_details.cache_write_tokens`。
写入缓存不等于命中缓存 —— 读错的表现是用户在**第一次**调用（最贵的那次）就看到满额命中。
我们本来就没读它，但现在有测试守着。

**仍未验**：`maxContextTokens`（两家都是）· 端到端"在 App 里选中这条模型 → 拖一张图 →
模型真的看见了"（这一轮验的是上游与能力位，不是 App 那条链路）· `deepseek-v4-flash` 未复测，
保留 2026-09-05 的 `imageInput: false`。
