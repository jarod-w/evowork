...
> **更新于 2026-09-25（第 55 次）**。这份文件回答一个问题：**现在到哪了、下一步是什么、什么还不能信。**
...
| 内核基线 | 断言基线 `d583e73c4d12`（2026-09-24）。本轮已重新核对 Goal（`tokenBudget` + 6 状态）、Queue update/reorder、searchOccurrences 与 `thread/goal/cleared`；F3 检测器也已跟随上游实现重写，不再把“仍只有 Plan + Default”误报为断言破坏。**F1–F25 已全部纳入机器断言**，支持锚点局部范围与仓内接线检查；当前 `BROKEN 0`。 |
