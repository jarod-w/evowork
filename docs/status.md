...
> **代码现在长什么样（进程 · 包 · 八条跨边界通道 · 守卫）在 [architecture.md](architecture.md)**（2026-09-26 按 `00209d8` 重写），
> 怎么编译与部署在 [build-and-deploy.md](build-and-deploy.md)。
> 这里只写**当前事实**，不写计划理由 —— 两边说法冲突时，以本文的"验收凭据"列为准。
>
> **第 62 次只回写文档，没有新功能。** 对照 `00209d8`，把总纲、架构、优先级和部署手册里已经不成立的「尚未实现」改成代码事实。电脑操控仍是下面里程碑表里的 🟡：协议、宿主、Helper 源码和 macOS release 装配在，`releaseVerified=false`，不能启用。前面第 47–52 次写「电脑操控仍是未开始」，那是当天的决策记录，不是现在的状态。
>
> 同一次订正一处过强表述：网关合并结果仍保留带 `denied` 的模型，请求返回 403 `model_denied`。设置页不再画这张表。`toModelOption` 会把 `denied` 放进下拉数据，`ModelSelect` 目前不据此禁用。不能把「设置页不展示停用原因」写成「目录里已经没有这些模型」，也不能写成「Composer 已经把它们滤掉」。
>
...
下面是当天的实现记录。其中「原生源码未编译」在同日后续的 release 装配之后已经过时；当前剩余项只维护在 [设计 12 §17](design/12-computer-use.md#17-未完成清单与验收条件2026-09-24)，不在这里再留一份会漂的清单。

- 已实现本机 stdio MCP、私有 socket 长度帧/令牌/序号、桌面宿主准入与任务生命周期、MCP 选项审批、设置页启停/撤权和活动停止入口。应用授权绑定签名；自动化和子任务拒绝；结构化审计不保存图文正文。
- 新增 macOS Swift Helper 源码与构建入口，包含 AX、窗口截图和部分写动作。同日稍后在 macOS 上完成 arm64 release 装配、纯策略测试和长度帧 `health` 握手。签名、公证、TCC 和实机中断仍未验收。拖拽、滚动、坐标回退与扩展选择仍拒绝，浏览器回退尚未实现。
