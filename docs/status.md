...
> **更新于 2026-09-19（第 37 次）**。这份文件回答一个问题：**现在到哪了、下一步是什么、什么还不能信。**
...
| 测试 | **1576 个通过、2 个跳过**（2026-09-19 实测，106 个文件）。跳过的是 runtime-installer 的两条联网端到端安装测试。**"没装扩展怎么办"那条不跳** —— 它由夹具强制构造（2026-09-06 修好，此前那个夹具名不副实，见 §3） |
