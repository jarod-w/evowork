...
> **更新于 2026-09-21（第 42 次）**。这份文件回答一个问题：**现在到哪了、下一步是什么、什么还不能信。**
...
>
> **第 42 次是一次产品决策回写，没有代码改动**：Composer 一级模式从 Craft / Plan / Ask
...
> 抓到一条**当前就已经被违反**的：identity 的 `/v1/admin/*` 在 `mustChangePassword`
> 为真时一律 403，而 `apps/web` 里 `/v1/password` **没有调用方** —— Q38 引导出来的
> 种子管理员登录成功、每个管理动作都被拒、界面上没有任何地方能改密。同轮登记：
> 管理端三笔欠账（成员动作靠手粘 `userId` · 策略包只显示 `kid` · 无管理动作审计）·
> 分享页的两条结构约束 · **新增 Q43（管理端用量只给聚合、不给按人按天曲线，已决策）**
> 与 **Q44（企业私有源索引的管理面做到哪一层，未决策 —— 当前唯一开放项）**。
> 验收口径从 22 条加到 25 条，**三条都还没有测试**。
>
...
**先说三条最要紧的剩余项**（E 于 2026-09-20 对账时新增）：
...
| E | **WEB 没有改密出路，种子管理员进不了管理端** | `services/identity/src/http.ts:326` 在 `mustChangePassword` 为真时对 `/v1/admin/*` 一律 403；`/v1/password` 全仓库只有 identity 自己的测试在调；`apps/web` 的 `AdminPage` 只判 `role` 不判 `mustChangePassword` | 11 §13.8 写的是「改完之前管理端只给改密」，**服务端那一半做了、WEB 那一半没做**：登录成功 → 每个动作 403 → 界面上无处可改。这是「界面在陈述一件不成立的事」那一类，且**第一台交付出去的部署就会撞上** |
...
| **11 账号与模型**（WEB 侧，2026-09-20 对账） | E（改密页，验收口径第 23 条**当前已被违反**）· 管理端成员动作靠手粘 `userId`（表里显示的是邮箱，id 无处可取）· 策略包只显示 `kid`，无可读 payload / 历史 / 撤销 · 无管理动作审计（谁何时授予 admin、改 key、签发策略包）· 管理端用量聚合视图（**Q43=A：只给租户总量 + 按人当期累计，不给按人按天曲线**）· 分享页 `/s/<share-id>`（Q41，跟 D 同一切片；**不读账号会话、不带 `authorization` 头**）· 企业私有源索引的管理面（**Q44 未决策 —— 当前唯一开放项**，推荐"只注册源、不托管内容"） | `apps/web/src/screens.tsx` · `services/identity/src/http.ts:326` · 细则 [11 §13.10](design/11-account-and-models.md) |

**仓内为空、但设计依赖的目录**：`ext/` · `config/showcase/` · `config/permissions/`（都只有 README 或 `.gitkeep`）。`plugins/agents/` 故意不预置角色。`apps/web/` 与 `services/identity/` 已有实现；**分享页与改密页仍未做**（见上表第 11 篇）。
