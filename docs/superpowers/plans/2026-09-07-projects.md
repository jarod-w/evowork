 ...
 * 01 §5.34 Dialog（模态壳）。
 *
 * **仓库里唯一的模态实现**。它不管自己什么时候出现 —— 由调用方决定挂不挂载，
...
在 `app.css` 里把现有的 `.ew-delete-confirm, .ew-revert-confirm { ... }` 那段**替换**为：
...
CSS 里 `.ew-delete-confirm-body` 已在 Step 5 保留（挂在 `.ew-revert-confirm-body` 那条选择器组里），
若被删掉就补回一条 `.ew-delete-confirm-body { margin: 0 0 var(--space-12); color: var(--text-secondary); }`。
