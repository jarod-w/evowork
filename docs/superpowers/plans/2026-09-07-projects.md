...
- Create: `services/projects/tsconfig.spec.json`
...
`services/projects/tsconfig.spec.json`（**别漏这个**：没有它 `pnpm typecheck` 的
「② 测试类型检查」那一遍会**跳过整个包**，于是这个包的测试文件从来没被 tsc 看过 ——
`strict` / `exactOptionalPropertyTypes` / `noUncheckedIndexedAccess` 对它全都不生效，
而这三条正是这个仓库赖以立住的东西。七个有测试的兄弟包都有这份文件）：
...
```ts
   ...
   */
  const agentsMemoPath = async (
    ports: ProjectPorts,
    root: string,
  ): Promise<string | undefined> => {
    const realRoot = await ports.realpath(root);
...
```ts
    ...
    async listProjectDir(input: {
       ...
       */
      const real = await ports.realpath(target);
    ...
    <div className="ew-page">
      ...
      {pending?.kind === 'remove' ? (
        ...
        >
           ...
           */}
          {/* JSX 里 markdown 的 ** 会原样渲染成星号 —— 强调要用 <strong>，sidebar.tsx 就是这么写的 */}
