...
| 项               | 内容                                                                                                                                                                                             |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 版本             | v0.13（按 `00209d8` 回写实现进度；没有新的产品决策）                                                                                                                                          |
| 日期             | 2026-09-26（v0.12 于 2026-09-24；v0.6 于 2026-09-21）                                                                                                                                          |
| 作者             | li.wang                                                                                                                                                                                          |
| 状态             | Q1–Q43、Q45 已决策，**当前唯一开放项为 Q44**；电脑操控 **CU-Q1–CU-Q6 已全部确认**。Q45 **已接线**。电脑操控**部分实现、发布关闭**（`releaseVerified=false`），不是「只有设计」也不是「已可用」 |
| 本轮修订（v0.13） | 纠正 v0.12 里两处已过时的实现陈述：§10.1.5 的「Q45 尚未实现」、§10.1.6 的「CU-M0–CU-M4 均未实现」。代码事实以 [status.md](status.md) 与 [architecture.md](architecture.md) 为准，本文不复制验收数字 |
| 本轮决策（v0.12） | **CU-Q6=A**：所有终端/命令行前端与 System Settings 整个 App 对 Computer Use 硬禁止，任何授权档不可覆盖；命令走既有 shell，系统设置交还用户                                                   |
| 本轮决策（v0.11） | **CU-Q5=A**：AX 正文与截图随本机任务历史留存并在首次使用前明确告知；归档不删除，真实删除任务必须清理 rollout、blob 与索引；结构化审计不复制正文                                               |
| 本轮决策（v0.10） | **CU-Q4=A**：支持浏览器 App，但普通网页优先 browser connector；Computer Use 只补 CDP 不可覆盖界面，并继承 origin、上传、下载、提交和数据外发策略                                             |
| 本轮决策（v0.9） | **CU-Q3=A**：允许用户选择“始终允许某 App”；只持久化应用准入，本机保存且可撤销，不跳过动作审批与硬禁止；未采纳 C 的“企业版一律禁用”                                                           |
| 本轮决策（v0.8） | **CU-Q2=A**：原生 Helper 与 `cua_repl` MCP server 随 macOS 基础包分发，但能力默认关闭；安装时不申请 TCC 权限，首次明确使用时才引导授权                                                           |
| 本轮决策（v0.7） | **CU-Q1=A**：电脑操控首版仅支持 macOS 14.4+；Windows UIA 与 Linux Wayland/X11 驱动作为后续独立能力，不进入首版                                                                                    |
| 本轮决策（v0.6） | **Q45**：Composer 常显模式从 Craft / Plan / Ask 换成 **请求批准 / 帮我批准 / 完全访问**。清单第三章的三项不再作为用户可选项；D8「不新增 ModeKind 枚举」仍成立                                                                 |
| 评估基线         | 机器断言基线现为 **`d583e73c4d12`（2026-09-24）**，F1–F29 见 [详细设计集 README §4](design/README.md)。本文附录 A 的目录路径仍按 2026-09-05 的 `89a4eec6da` 核对，动手前以漂移雷达为准 |
| 输入文档         | `agent-platform-feature-list.md`（EvoWork 功能点清单）· [详细设计集 01–12](design/README.md)                                                                                                     |
...
>
> **v0.13**：桌面发送链路已按上表展开（`bridge.send` 带 `modeId`）。`auto_review` 只对假 app-server 测过，没有对着真实内核跑过一轮。完全访问在 Windows 上按能力描述停用。
...
| **定时执行 / Automations**  | **[自建]** | **codex 全仓库无 cron/scheduler**。本机调度器已落地；`wake_system` 的系统唤醒仍未接 |
| 可视化（SVG/HTML/Chart.js） | [自建]     | 无渲染层（本属前端职责）。桌面 Visualizer 已落地，见 [architecture.md](architecture.md) |

记忆这一行是能力分类（内核有，我们复用），不是设置页的完成声明。桌面「设置 → 个性化」已经接到 `config/read`、`config/batchWrite`、`memory/status`、`memory/reset` 和 `thread/memoryMode/set`。EvoWork 不读记忆正文，也不承诺和 ChatGPT 云端同步。V1/V2 的状态展示边界见 [status.md](status.md)。
...
| **资料库（一级入口）**              | **[自建]** | **v0.4 新增行**：清单把它拆散在第 6 / 12 章，但真实产品里是与「任务」同级的一级入口（三栏 + 索引 + 全文搜索）。Codex 记忆控制在「设置 → 个性化」，资料库不直读内核生成文件。工作量见 §8 M8，设计见 [06 资料库](design/06-library.md) |
...
| 模型网关（D2）                             | 密钥托管、计量、三家国内模型适配（Q2/Q16） | prompt 过境但**不落盘**；Q14：云端托管为主 + 企业私有部署包（客户自持密钥）。**同一网关还有一个能力端点** `GET /v1/evowork/models`，桌面 App 的模型下拉直接读它（F24）—— 这是 K6 意义上的一条出网路径，但它**只发一个 GET、不带任何用户内容**（没有 prompt、文件名、任务 id），只带一个 Bearer 令牌 |
| 产物分享托管（Q10=A）· 审计汇总 · 策略下发 | 对外分享与企业合规必须有服务端             | **默认关闭**，逐次显式授权，链接有有效期                                    |

**K6 登记：办公扩展的下载（2026-09-07 新增）**。这是四类云端职责之外的一条出网路径，登记在此：

| 项       | 内容                                                                                                                    |
| -------- | ----------------------------------------------------------------------------------------------------------------------- |
| 触发     | **只有用户在引导第 ④ 步或提示里点「安装」时**。没有任何自动/后台下载                                                      |
| 去哪     | `github.com`（python-build-standalone 发行版）· `mirrors.tuna.tsinghua.edu.cn`（六个 python 包的 wheel，清华 PyPI 镜像）。中文字体 **随基础包分发**，安装时不再出网 |
...
| 不出网时 | 企业用 `EVOWORK_OFFICE_BUNDLE` 指向离线包（`scripts/build-office-bundle.mjs` 打），**这条路径一个字节都不出网**，有测试守 |

> **为什么它不算破 K6**：K6 管的是"用户的原始数据不离开本机"。这条路径的方向是**下行**，
...
| 触发           | 去哪                        | 带什么                                                    | 不联网时                                                                                       |
| -------------- | --------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 用户点「登录」 | identity 授权端点（系统浏览器） | PKCE challenge · `deviceId` · 客户端版本。**无用户内容**   | 登录失败并明确提示；**只用自定义模型（BYOK）的用户根本不需要这条路径**（Q30=A）                 |
| 令牌刷新       | identity token 端点         | refresh token · `deviceId`                                | 默认模型不可用（如实说明，有自定义模型时给切换动作）；策略包按 R11 的宽限与超期降级             |
| 计量上报       | identity 计量端点           | **按天按模型聚合**：`sub` / `tenant` / `model` / token 数 / 时延 / 错误码。**没有 `threadId`** | 本机排队，**上限内丢弃而不是无限堆积**（一个只增不减的队列是另一种数据留存）                    |

> `threadId` 是这张表里最容易被加进去的字段，也是必须挡住的：它看起来只是个随机 id，
> 但带上它云端就能按任务重建用户的活动时间线（几点在干活、一个任务几轮、什么时候失败）——
> 那已经是内容面，而这一行承诺的是"无内容"。**未登录时这三条路径一条都不走**（Q30=A，
> 验收口径见 11 §12 第 14 条）。
...
### D10 —— 账号是**凭据**，不是数据归属主体（2026-09-08，Q30–Q37 的地基）

**决策**：账号解锁 D9 的四类云端职责，**但不改变数据归属** —— 任务、产物、自动化、审计属于**这台机器**，不属于账号。落地形式：本机 sqlite 的任何表都不引入 `user_id` / `tenant_id`（Q31=A）；登出只清令牌，一行本机数据都不动；不做跨设备同步（Q17 / Q19 已定）。隔离边界是 **OS 账号**，不是 EvoWork 账号 —— 多人共用一台电脑就各用自己的 OS 账号，产品内不提供第二层隔离。

**理由**：这是 D9 末句「后续任何设计若需要"服务端在用户离线时替他做事"，必须先回到本条重新决策」在有账号之后唯一能自洽的形态。把账号做成数据归属主体，"云端存一份方便跨设备"就成了顺理成章的下一步，而那会依次推翻 Q17、Q19、K6 与 D9 本身（这就是 R12）。

**机制化（不靠自觉，三条都是会失败的检查）**：

1. `packages/logging` 不注册 `userId`（要关联就用 `sub` 的摘要或 `tenantId`），计量上报的类型里**没有 `threadId` 字段**；
2. **schema 扫描**：`services/store` 的测试遍历两个迁移器的全部 DDL，出现 `user_id` / `tenant_id` 列即失败 —— 与 K6 在 `services/ingest` 的整目录扫法同一形状。2026-09-08 首次跑它就抓到 `automation` 表里两个从没有人读写的 `tenant_id` / `owner_id` 列（迁移 v3 已删）；
3. 登出路径有断言钉住"本机表行数不变"。

### D11 —— 本机网关是唯一的 provider 端点，上游按模型路由（2026-09-08）

**决策**：内核的 `model_providers.evowork.base_url` **恒为 loopback**；本机网关常驻，按 model id 决定上游 —— 自定义模型用本机密钥直连厂商，管理员配置的默认模型转发到云端网关。拓扑（`~/.evowork/app.toml` 的 `mode`）描述的是"**默认模型的上游在哪**"，不是"网关在哪"。
...
基线来源：[OpenAI Docs · Memories](https://learn.chatgpt.com/docs/customization/memories)（复核于 2026-09-26）。
EvoWork 只在“默认开启”上按本产品决策偏离上游的默认关闭；其余控制语义与数据边界以该基线为准。
...
2. **解析运行时按档位分发**（对齐 R10「Python 按需下载而非全量随包」）：基础包（TXT/MD/CSV/TSV/ZIP/JSON + **Noto Sans SC 中文字体**，随主程序）· 办公扩展（Python + python-docx/openpyxl/python-pptx/pdfplumber，~120MB，按需下载）· OCR 扩展（tesseract + 中文模型，~60MB，按需下载）。企业离线部署提供全量包。**产物生成技能与解析管道共用同一档位**，提示文案必须统一（08 §4）。字体从按需下载里拆出，是因为客户机器经常打不开 GitHub raw。
...
| 单任务 token 预算                  | **硬预算**；**定时任务不得不设**（v0.4 修订，07 §3.2）                                                                   | `ThreadGoal.tokenBudget`，无需自建                                                                  |
...
| 单任务预算 | `ext/goal` 的 `ThreadGoal.tokenBudget`                    | 与上面 Q11 是同一套机制，不重复造    |
...
| **M8 可视化 + 产物分享 + 资料库**                | Visualizer + 本机产物索引 + 分享链接（Q10：默认关闭、逐次授权、有有效期）+ **资料库主体**（三栏 + 索引 + FTS 全文搜索，v0.4 范围增量，Q28） | **8** | 记忆按当前 Codex 基线另放「设置 → 个性化」 |
| **M9 桌面分发**（Q1=A 新增）                     | 打包（内核二进制 + Python/Node 解析运行时）、三平台签名/公证、自动更新、崩溃回收、EvoWork CLI 随包（Q13）                                                | 3     | 三平台安装包可自动升级，离线可用                                                       |
| **M10a 模型管理**（2026-09-08 新增，[11 §10](design/11-account-and-models.md)） | `safeStorage` 密钥库 + 设置页 + 模型注册表的 ① / ③ 两层 + 私有 endpoint + `SecretInput`（01 §5.35）+ `app.toml` 的 `mode`                                | 1–1.5 | 密钥不落明文盘（`secrets.bin` 密文 + 系统钥匙串）；`~/.evowork/gateway.env` 退役       |
| **M10b 账号**                                    | `packages/account` + identity MVP（邮箱密码 + 邮件验证，Q32=B）+ `apps/web/` 账号页与管理端 MVP + 租户模型（Q37=B）+ 网关 JWT 验签 + 计量 + 本机网关转发模式 | 3.5–4.5 | 上游 B 可用；租户管理员能配默认模型；未登录仍可用 BYOK（Q30=A）                        |
| **M10c 企业面**                                  | 配额策略 + 签名策略包下发（**与 M4 的两个遗留项合并**；租户已前移到 M10b）                                                                               | 1.5   | R11 缓解手段落地；企业形态可交付                                                       |

合计约 **57 人周**（≈ 4 人 × 3.6 月）到达本期范围对齐；**M10 三段另计 6–7.5 人周**（2026-09-08 新增，此前「账号与模型管理」没有任何里程碑承载它，见 [11 §2](design/11-account-and-models.md)）。**M10a 已实现**（2026-09-08），它不依赖 M10b，也没有任何外部前置条件。

**实现进度（2026-09-26，v0.13）**：上表是当时的估算，不是剩余工作量。M0–M10c 的核心代码已经落地，WEB 缺口收口（P3.5-4）也已落地。仍未完成或未开放的，以 [status.md](status.md) 为准，这里只列不会被「代码写过」盖掉的几项：

- 电脑操控：协议、宿主、Helper 源码和 macOS release 装配在仓库里，`releaseVerified=false`，不能启用。CU-M0–CU-M4 都还没达到 [12 §16](design/12-computer-use.md) 的完成定义。剩余项是 CU-R1–CU-R11。
- 分享上传与云端托管、资料库正文检索、OCR、`wake_system` 的系统唤醒、通知中心、语音、公开远程市场。
- 外部验证：U1 产物质量、U3 misfire 真机、U4 签名公证、U5 Windows 隔离、U6 `safeStorage` 真机。Q44 仍未决策。
...
| R7  | 隐私承诺（"原始数据不上传"）与云端能力（分享链接、云端多模态）冲突                                        | 高   | **低**（已收敛） | Q1=A + Q3 硬约束把执行面整体留在本机；本机 Codex 记忆不与 ChatGPT 云端记忆同步。剩余新增出网点只有分享链接（Q10）；模型调用这条必然出网的路径由 Q14 的「网关不落盘」承诺兜底，且该承诺需可审计（配置固化 + 日志抽查） |
...
| R10 | **桌面分发成本**：安装包含内核二进制 + Python/Node 运行时（+100–300MB）、三平台签名公证、自动更新、内核随上游升级 | 中   | 高               | 单列 M9；Python 办公运行时按需下载而非全量随包；**中文字体随基础包分发**（客户上不了 GitHub raw，约 18MB）；升级走差量包                                                                                                                                                                                                                                                                                                                                                      |
| R11 | **治理弱化**：Q1=A 后企业策略无法在服务端强制拦截，只能下发到每台设备                                             | 中   | 中               | 策略包签名 + 启动时校验 + `requirements.toml` / `allow_managed_hooks_only`；离线超期自动降级为只读模式                                                                                                                                                                                                                                                                                                     |
| R12 | **账号引入后 D9 的边界被逐步侵蚀** —— 「既然已经有账号了，任务列表 / 产物索引 / 记忆存云端也顺理成章」 | **高** | 高（推翻 Q17 / Q19 / K6，且不可逆） | D10 的三条机制化手段（schema 扫描 · 不注册 `userId` · 登出后行数不变）；任何新增云端数据面职责必须回到 D9 重新决策并在本文留痕                                                                                          |
...
### 10.1.4 账号与模型管理 Q30–Q37（2026-09-07 决策，2026-09-08 回写自 [11 §9 / §13](design/11-account-and-models.md)）

八条的共同前提是 **D10**（账号是凭据，不是数据归属主体）。**Q30 / Q31 / Q34 / Q37 四条直接影响写码方式**，已同步进 CLAUDE.md 第 8 节。

| #   | 问题                     | 决策                                                                                                                       | 为什么这样定 / 落在哪                                                                                                                                                     |
| --- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Q30 | 未登录能否使用产品       | **A 能用**，BYOK 与登录**并列**；未登录可用全部执行面，**但只能用自定义模型**                                              | Q1=A 的整个叙事是本地优先，强制登录会把一个不需要联网的执行面变成需要联网的。机制化：未登录时对我们的域名**零请求**（连模型清单都不拉），拔网线后自定义模型仍可用           |
| Q31 | 多账号共用一台机器       | **A 不做产品内隔离** —— 隔离边界是 OS 账号                                                                                 | 直接推出 D10：本机表不引入 `user_id`；换账号登录后看到的还是这台机器原有的任务与产物，且**登录页必须写明这一点**，不让用户自己推断                                         |
| Q32 | 注册凭据形态             | **B 邮箱密码 + 邮件验证**为主路径（企业 OIDC SSO 不受影响，在 M10c）                                                       | 把 §10.1 里唯一可能长于开发周期的采购项（短信通道 + 实名资质）换成发信域配置（天级）。代价：identity 多一套密码职责（argon2id · 限速锁定 · 一次性令牌），+2~3 人日         |
| Q33 | 登录交互形态             | **A 系统浏览器 + OAuth Code + PKCE + loopback 回调**；device code 不做                                                      | 与 Q32=B 合起来是硬约束：**密码只出现在 WEB 的表单里**，客户端进程与 IPC 里没有 `password` 字段。内嵌 webview 是改不掉的钓鱼面，且多数企业 IdP 直接拒绝它                   |
| Q34 | 密钥与令牌存哪           | **A Electron `safeStorage`**（Keychain / DPAPI / libsecret），密文落 `~/.evowork/secrets.bin`；不可用时**显式让用户选**     | 明文文件与 Q14「密钥托管」的卖点冲突，且 refresh token 落明文等价于账号落明文。**已随 M10a 实现**：`gateway.env` 迁移后改名 `.migrated`，密钥不读回渲染层，只显示后四位   |
| Q35 | 计量与配额的上行内容     | **A 只上行身份 + 聚合计量**，`threadId` 与任何自由字符串都不上行                                                            | 见 D9 新增的出网路径登记：带上 `threadId` 云端就能重建活动时间线，那已是内容面                                                                                             |
| Q36 | BYOK 与托管能否共存      | **A 能，同一台机器上并存**，靠每条目录条目的 `credentialSource`（`byok` / `hosted` / `private`）区分                        | 这条与"内核只有一个 provider base_url"冲突，解法就是 **D11**（本机网关按 model id 路由上游）。字段对用户可见，因为它同时回答"花谁的钱"和"数据过谁的境"                     |
| Q37 | 「管理员」是谁           | **B 租户管理员**（我们的运营 = 一个内部租户；私有部署 = 只有一个租户）                                                     | 一套代码覆盖三种形态。代价：**租户模型从 M10c 前移到 M10b**（+3~5 人日）。管理端在 `apps/web/`，**客户端没有管理界面**，且管理端 API 的类型里没有能装内容的字段           |

**这八条里有两条是"结构上不可能"型的机制**（同 K6 的整目录扫描、Q14 的字段注册表）：计量类型里没有 `threadId`（Q35）· 客户端目录条目类型里没有 `apiKey`（Q36 / §13.2）。不是靠约定，是想记就得先改类型，而改类型会被 review 看见。

**Q38–Q44 只记在 [11 §9](design/11-account-and-models.md)**，本节不复制一份（那个领域的决策真源是那张表）。其中两条要在这里点名，因为它们越过了账号这一块：
...
**写码时意味着什么**（2026-09-21 已接线；下面是现在还成立的约束，不是待办清单）：

1. Composer 常显三项是请求批准 / 帮我批准 / 完全访问，不再另设权限下拉。企业自定义 `[permissions.<id>]` 出现在同一菜单底部，未知 id 不隐藏（10 §2.2）。
2. 前端传 `modeId`（`request-approval` / `approve-for-me` / `full-access`）。适配层展开为 `permissions` + `approvalPolicy` + `approvalsReviewer`，`collaborationMode.mode` 固定 `default`。请求批准下发 `on-request`；完全访问下发内置 `:danger-full-access`，投影里仍可记产品 id `evowork-full`。命名档不能 `extends = ":danger-full-access"`，否则内核回 -32600。
3. `bridge.send` 已经把 `modeId` 带进 `turn/start`。不要再写成「发送链路不传权限、任务始终用场景包」。
4. 帮我批准在 `approvalsReviewer` 不可用时禁用并说明原因，**禁止**改成 `user` 发出去。`auto_review` 尚未对着真实内核验证。
5. 完全访问在 Windows 隔离未知时停用并给原因。10 §7 的本机安全能力页还没有，设置里的「安全与权限」目前只展示策略包状态。

### 10.1.6 电脑操控 CU-Q1–CU-Q6（2026-09-24，来自 [12 §15](design/12-computer-use.md)）

| # | 状态 | 决策 |
| --- | --- | --- |
| **CU-Q1** | ✅ 已确认 | **A：首版仅支持 macOS 14.4+。** Windows UIA 与 Linux Wayland/X11 驱动作为后续独立能力，不进入首版；跨平台协议占位不等于已有平台支持。 |
| **CU-Q2** | ✅ 已确认 | **A：随基础包但默认关闭。** Helper 与 MCP server 不走首次使用下载；安装时不申请 TCC 权限，用户首次明确使用时才启用和引导授权，企业 requirements 可继续禁用。 |
| **CU-Q3** | ✅ 已确认 | **A：允许“始终允许某 App”。** 持久授权只作用于 App 准入；本机保存、可撤销，不跳过动作审批、硬禁止或具体 App deny。未采纳 C 的企业版一律禁用。 |
| **CU-Q4** | ✅ 已确认 | **A：支持浏览器 App，但 browser connector 优先。** Computer Use 只补 CDP 覆盖不到的界面；所有 URL、上传、下载、提交和数据外发拒绝跨通道继承。 |
| **CU-Q5** | ✅ 已确认 | **A：AX 正文与截图随本机任务历史留存并明确告知。** 归档不删除；真实删除任务/清理历史时必须删除 rollout、图片/blob 与索引；结构化审计仍不保存正文。 |
| **CU-Q6** | ✅ 已确认 | **A：终端和系统设置全部硬禁止。** 在激活、AX、截图前拒绝，持久授权、审批和完全访问不可覆盖；命令走受控 shell，系统设置由用户手动操作。 |

电脑操控不是「只有设计」，也还不能用。协议、状态机、桌面宿主、MCP 壳、引导技能和 macOS Helper 源码已经在仓库里；2026-09-24 完成过 arm64 release 装配和长度帧 `health` 握手。构建把 `releaseVerified` 写成 false，宿主因此拒绝启用。CU-M0–CU-M4 都还没达到完成定义，剩余项是 [12 §17](design/12-computer-use.md#17-未完成清单与验收条件2026-09-24) 的 CU-R1–CU-R11。进程边界见 [architecture.md](architecture.md) 通道 ⑧。
