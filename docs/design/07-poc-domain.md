...
> **Q-27**：冻结快照放 git（会很大）还是放 blob store 加一个索引文件？建议后者，git 里只放 `case.yaml` 与 `truth/`。
...
| # | 问题 | 谁定 | 备注 |
|:-:|---|:---:|---|
| Q-24 | 用友取数走 API 还是数据库直连 | 客户 | 影响 [05 §5](05-execution-plane.md) 的出口管控路径 |
| Q-25 | 历史 3–6 个月人工成品表 + 只读账号 | 客户 | **M0 唯一的客户侧关键路径**，第一天就要开口 |
| Q-26 | 财务能否接受口径以文件形式维护并自己改 | 客户 | 不能的话口径库退化成工程师维护的配置 |
| Q-27 | 冻结快照存 git 还是 blob store | 团队 | 建议 blob store + 索引 |
