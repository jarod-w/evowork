...
- 连续的过程性 item（思考、操作、变更、协作、产物）与夹在中间的助手短回复，先归并为一个「处理过程」组；**运行中默认展开**，推理正文随组一起挂载，用户能看见生成过程。完成后收成一行摘要；点开即可看到推理，不必再点「推理过程」。命令输出与工具 JSON 仍二次折叠。过程组始终画在该回合最终助手回复之上——即使回复 item 先到。用户消息和该回合最后的助手回复直接铺开。
