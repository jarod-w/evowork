...
- 结果工作区**默认收起**（`⌘I` 切换）。用户点击产物、文件、diff 或网页预览时打开对应 Tab。
- 系统只在任务意图明确要求持续预览、比较或交互时主动打开；普通附件、单个下载文件、后台文件变更以及“第一个产物出现”本身都不是触发条件。
...
- 连续的过程性 item（思考、操作、变更、协作、产物）与夹在中间的助手短回复，先归并为一个**默认折叠**的「处理过程」组；用户点开后才挂载各条推理、命令与工具记录，各条的完整正文仍按下表二次折叠。用户消息和该回合最后的助手回复直接铺开。这样默认消息流只显示用户输入、最终回复和一行过程状态，不泄露整段内部执行记录。
- 新 item 插入做 120ms 淡入 + 4pt 上移（01 §2.7）；流式文本追加不做动画。
- 自动滚动：仅当用户已在底部时跟随；用户上滑后停止跟随并在右下角显示「↓ 有新内容」按钮。

### 5.2 逐类规范

视觉上先归并为一个默认折叠的「处理过程」组。思考、操作、变更、协作、产物和夹在中间的助手短回复都在组内；用户点开后才看到原始命令、JSON 与完整输出。下表保留 19 类协议级映射，确保未知或低频事件不丢失。
