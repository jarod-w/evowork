...
| 任务是否贡献记忆 | `thread/memoryMode/set` (exp)；总开关、使用与生成在「设置 → 个性化」 |
...
| 6   | `FileChange`                                | **展开**     | 变更卡：可点击文件路径 + `+n/-m` 统计 + 折叠的 diff。新增文件按任务 `cwd` 做路径规范化、真实路径与软链边界复核后在应用内只读预览；修改/删除文件打开对应 diff（首屏最多 40 行，超出「查看完整变更」跳结果区变更视图）。数据增量来自 `item/fileChange/patchUpdated` |
