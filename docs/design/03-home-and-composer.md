...
`turn/start` 成功后移除，启动失败不会丢输入。队列编辑和拖动重排仍未进入页面。
...
用户在下拉里选的模型按 §2.4 的优先级展开进 `turn/start`（用户显式选择 > 模式 > 场景偏好），
**并同时写进任务级设置** —— 只传不存的话，下一回合会从投影表读回旧值，
表现是"切了模型只生效一轮，然后悄悄换回去"。场景包里的 `model` 是偏好：
目录里有它就用，没有就用这台机器上第一个可用的模型（Q30 未登录只有自定义模型）。
不因此弹「场景默认不可用」——那句话留给用户自己选过的模型消失。
...
| 未选项目就添加本地文件                | 已有任务：落到该任务的 cwd/`uploads/`（任务已经在那个目录里跑，沙箱才能读到）。首页还没有任务：Toast 说明要先选项目，**不静默没反应**，也不偷偷写到别处 |
| 模型不可用（网关不通 / 未登录 / **没配模型**） | Composer 顶部插入 `--danger` 提示条。没配模型时按钮是「去设置添加模型」；网关暂时没起来时按钮是「检查模型接入」。发送按钮禁用。**不静默降级到其他模型** |
