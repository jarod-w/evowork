...
token 分发：daemon 首次启动生成，写在常开机器的配置文件里，管理员复制给财务。企业微信推送的审批链接里带一个**一次性短期 token**（只对该 approval 有效），这样手机点开就能批，不需要在手机上存长期凭据。**Q-22**
...
UI 与 daemon 版本不匹配时的处理，现在就要定，因为 POC 期 daemon 在客户机器上、UI 在浏览器里，**两者的更新节奏天然不同步**（刷新页面就换了 UI 版本，daemon 却没重启）。

建议：主版本不匹配 → UI 显示「请刷新 / 请联系管理员升级」并只允许只读操作；次版本不匹配 → 正常工作。**Q-23**
...
守住这两条，将来套 Tauri 外壳 = 重写那一个适配器，两天的事。

POC 期只出浏览器形态，`platform` 接口的浏览器实现基本是空的——但接口要在。
...
| # | 问题 | 谁定 | 建议 |
|:-:|---|:---:|---|
| Q-22 | token 分发方式；审批链接的一次性 token 有效期 | 团队 | 审批 token 24 小时 + 单次使用 |
| Q-23 | 协议版本不匹配的处理 | 团队 | 主版本不匹配降级为只读 |
