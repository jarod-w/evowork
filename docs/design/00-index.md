...
| 9 | **外壳不渗进业务代码** | `grep -rlE '@tauri-apps/\|ipcRenderer\|__TAURI' apps/ui/src/` 的命中必须全部落在 `apps/ui/src/platform/` 内。POC 期就要出桌面外壳（4.10②）之后，这条从「将来注意」变成当期防线——**UI 里到处 `invoke`，就是红线 1 的前置形态** |
...
>
> **为什么还额外匹配 `__TAURI` 前缀（不是精确匹配 `__TAURI__`）。** Tauri 2 注入 `window.__TAURI_INTERNALS__` 供底层 IPC 使用（`invoke()` 内部走的就是它）；`tauri.conf.json5` 的 `app.withGlobalTauri` 打开后还会额外注入 `window.__TAURI__`。曾经只精确匹配字面量 `__TAURI__`，并论证过它不是 `__TAURI_INTERNALS__` 的子串——但那个论证没有换来任何东西：`platform/` 本来就在检查里被排除，精确匹配唯一的实际效果，是漏掉了业务代码里直接写 `window.__TAURI_INTERNALS__.invoke('plugin:dialog|open', ...)` 这条最直接的绕过路径（完全不含 import 语句，`@tauri-apps/` 那半条匹配不到）。放宽成前缀匹配后，`platform/index.ts` 里合法的 `'__TAURI_INTERNALS__' in window` 探测不会被误伤，**原因是它在被排除的 `platform/` 目录里，而不是因为字符串不匹配**。
