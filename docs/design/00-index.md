...
| **Q-29** | **演示时刻 1 的话术怎么改** | 「财务明细一条没出内网」在 Q-01e 之后是错的。三个卖点里第一个被削弱，需要客户与团队都对齐。**这条要改的是 POC 文档第五节，不是设计文档** | 04 |
...
| ~~Q-21~~ | ~~常开机器平台与公网可达性~~ | **已定：macOS，可访问公网模型 API**。Windows daemon 的排期风险与 08 vendor 路径的前提同时消掉 | 05 / 08 |
...
| Q-22 | UI ↔ daemon token | 常开机配置文件分发；**审批链接 token 24 小时 + 单次使用** | 06 |
