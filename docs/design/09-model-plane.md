...
**这条区分是硬约束，不是习惯**：客户只与 DeepSeek 签了不做训练的协议。GPT 是开发期供应商，**交付到客户机器上的部署里，出口白名单只有 DeepSeek 一条**——[05 §4](05-execution-plane.md) 的 allowlist 就是这条约束的执行机制，不需要额外代码。
...
| **Q-01e** | **「财务明细进模型上下文」客户能接受到什么程度** | **客户** | 「不做训练」是合同承诺，不等于「数据不出内网」。做法见 [04 §3](04-context-memory.md)，但边界要客户点头 |
