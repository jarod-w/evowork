...
> **演示时刻 1 的话术必须跟着改（Q-29）。** POC 文档第五节现在写的是「财务明细一条没出内网」——Q-01e 之后这句话是错的，客户 IT 会当场指出来。可当场验证的说法变成：**除 DeepSeek API 外没有任何出口，且发出去的每一段内容逐条可查**。
>
...
| # | 问题 | 谁定 | 建议 |
|:-:|---|:---:|---|
| **Q-29** | **演示时刻 1 的话术怎么改** | 客户 + 团队 | POC 文档第五节「财务明细一条没出内网」在 Q-01e 之后是错的。建议改为「除 DeepSeek API 外没有任何出口，且发出去的每一段内容逐条可查」，见第三节 |
