...
> **演示时刻 1 的话术已跟着改（Q-29 已闭环）。** 它从「数据没走」更名为「**出口可控、去向可查**」，改演两份日志：出口日志（只有 allowlist 那三条）+ 本节这份内容清单，并当场增删一条 allowlist 让客户看见管控是活的。
>
> POC 三个卖点里第一个被削弱了，另两个的权重相应上升——这件事要向团队讲清楚，否则会有人仍按「零外发」的假设去做取舍。
...
| # | 问题 | 谁定 | 建议 |
|:-:|---|:---:|---|
| ~~Q-29~~ | ~~演示时刻 1 的话术怎么改~~ | — | **已改**：更名为「出口可控、去向可查」，改演两份日志（出口日志 + 发给模型的内容清单）并当场增删 allowlist。POC 文档共 6 处同步收紧 |
