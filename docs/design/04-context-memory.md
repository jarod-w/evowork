...
## 三、上下文最小化：明细不进上下文，进沙箱

> **Q-01b 已定：模型为 DeepSeek（云端 API）。** 这一节因此从「好习惯」升级为「演示时刻 1 能否成立的前提」。

进模型上下文的内容会离开客户内网。上游签了不做训练的协议，但那是**合同承诺**；功能清单划过这条线：「数据不出本机」是可**验证**的承诺，云端只能给合同承诺。

POC 文档演示时刻 1 的原话是「财务明细一条没出内网」。**这句话只在明细确实没进 prompt 时才成立。**

### 做法：模型不做算术，沙箱做

| 进上下文的（会出内网） | 留在沙箱的（不出内网） |
|---|---|
| 数据 schema（字段名、类型） | 用友取回的全量单据明细 |
| 口径库（账龄分档、不催清单规则） | 中间计算过程 |
| 少量脱敏样本行（供模型理解结构） | 产物文件内容 |
| 计算脚本本身 | Run Log 与 blob store |
| **聚合结果**（分档金额、条数） | |

Agent 的正确形态是**写一段脚本在沙箱里算账龄**（`class = Compute`），而不是把 247 条单据塞进上下文让模型加总。

这不是新机制，是 Effect class 的正常用法。它同时给三件事：

| 收益 | 说明 |
|---|---|
| **出内网的内容大幅缩小** | 演示时刻 1 那句话变成真的 |
| **准确率** | 4.7 确定性层要求 **100%，差一分钱算失败**。让模型做几百行求和是自找的——脚本不会算错 |
| **成本** | 247 条单据不进上下文，token 直接省掉 |

> 反过来说：如果 Agent 被允许把明细读进上下文再心算，那么确定性层 eval 迟早会挂在某个月的某一格上，而那时你会以为是模型不行，其实是架构让它去做了不该它做的事。

### 「出内网内容清单」：Run Log 的又一个免费投影

`context.assembled.blocks` 已经记了每一块的 `source` / `content_hash` / `token_estimate`（[01 §4.3](01-run-log.md)）。于是可以直接渲染一张表：

> **本次任务发给 DeepSeek 的全部内容**：口径库 3 段、数据 schema 1 段、样本 5 行、聚合结果 12 个数字。单据明细 0 条。

**演示时刻 1 因此可以从「断网还能跑」升级为「逐条列出发出去了什么」**——对财务客户，后者的说服力高一个量级，而实现成本是零：数据已经在 Log 里。

### DLP 的挂载点

「哪类数据不得进入模型上下文」这条 P0（POC C 类不做）将来就挂在装配器这一层：一条规则拦住 `trust = OrgTrusted` 且带客户名的明细块。**POC 期不做规则，但装配器是唯一入口这件事现在就成立。**
