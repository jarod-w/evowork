...
macOS Unix socket 路径长度有限：用户数据目录路径超限时，宿主改用 `/tmp` 下随机命名的私有 `0700` 目录，
socket 仍为 `0600`，并在退出时清理；这只改变本机 socket 路径，不改变令牌与授权校验。
...
- Electron 宿主监听 `~/.evowork/run/<pid>-<random>/cua.sock`；路径过长时使用上述 `/tmp` 私有目录；目录权限 `0700`、socket `0600`。Helper 只使用宿主独占的 stdio。
...
- 新增 macOS Swift Helper 源码与构建入口，包含 AX、窗口截图和部分写动作。同日稍后完成 arm64 release 装配；签名、公证、TCC 和实机中断仍未验收。拖拽、滚动、坐标回退与扩展选择仍拒绝，浏览器回退尚未实现。当时写的「原生源码未编译」已过时，以 §17 的 CU-R1 为准。
...
## 17. 未完成清单与验收条件（2026-09-24）

实现基线：`94c72b9`（基础提交 `471e222`）。**CU-M0–CU-M4 均未达到整体完成定义**。
下表是剩余工作的跟踪入口；前文描述目标行为，不代表已经交付。优先完成 CU-R1/R2/R3/R4/R8 的技术与安全前置，再完成产品收口，所有发布阻塞项关闭后才可开放能力。

| 编号 / 里程碑 | 当前事实与未完成项 | 关闭条件 |
| --- | --- | --- |
| CU-R1 · M0/M1：原生编译 | **原生 release 编译、纯策略测试及进程 health 握手已通过，读状态链路待验**。2026-09-24 在 macOS 27.0 / CLT 27.0、Swift `6.4.0.34.1` 与 macOS 27.0 SDK 上完成 arm64 Helper `.app`；沙箱缓存权限引发的 SDK 不匹配误判已排除。Swift 策略测试覆盖窗口唯一匹配、文本重复拒绝、UTF-16 范围和扩展选择；真实 Helper 响应长度帧 `health` | 验证 AX-only 与窗口截图的真实读状态链路、权限拒绝/撤销和目标 macOS 14.4+ 版本；补 UI 相关原生测试 |
| CU-R2 · M1：双向认证与组件生命周期 | **部分实现**。宿主验证 Helper 签名及相同 Team ID；Helper 对调用方的 code-signature/audit-token 第二因子未实现。进程/私有通道已有代码，重启、断连、超时和版本不匹配仍需端到端验证 | 拒绝伪造调用方、错误签名/版本；Helper 或宿主退出后授权失效；升级与恢复流程可复现，组件异常如实展示 |
| CU-R3 · M2：完整动作面 | **部分实现**。元素单次左键点击和 `select_text` 的 `replace`/`extend` 已有代码；扩展选择仅通过纯逻辑测试，真实 AX 未验。坐标点击、多次/其它鼠标按钮点击、滚动与拖拽未完成。工具 schema 不能作为动作已实现的凭据 | 九个写动作全部实现并通过原生测试；坐标回退严格绑定截图/窗口，元素、窗口移动与过期状态均拒绝；文本输入不偷换为按键执行 |
| CU-R4 · M2：中断与恢复 | **已有代码，待真机验证**。物理输入监听、Esc、停止、空闲退出、预算和无变化检测已有实现；剪贴板崩溃恢复与中断可靠性尚未证明 | 用户输入/锁屏/焦点切换/Helper 崩溃等矩阵通过；中断 P95 < 500 ms；无悬挂鼠标/修饰键；剪贴板不覆盖用户新内容；预算与无变化循环真实停机 |
| CU-R5 · M1/M3：可信上下文与企业策略 | **待补完整接线**。宿主目前用非自动化任务判交互来源；凭据来源是通用占位说明；`imageSupported=false`；存在 requirements 文件即整体禁用 | 从可信上下文识别桌面交互根任务，未知来源拒绝；接真实模型能力/凭据去向与本机、企业策略合并；模型或凭据变化重新告知；企业 deny 不被持久授权覆盖 |
| CU-R6 · M1/M3：App 识别与浏览器回退 | **待实现**。原生仅允许 TextEdit、Finder、Numbers；通用可信类别识别、browser 优先路由与受控回退未完成 | 扩展 App 范围前验证身份/类别识别；终端、系统设置、自身审批界面及敏感 App 在激活/读取前拒绝；浏览器 origin/上传/下载/提交拒绝跨通道继承 |
| CU-R7 · M3：真实内核与审批闭环 | **接线与引导技能已有，端到端未验**。stdio MCP、宿主同意卡、内核 elicitation 适配及 `plugins/skills/computer-use` 已实现；三档动作审批/自动审查对真实 Computer Use 调用尚未验收 | 正式内核验证握手、可信调用元数据、技能触发、工具发现/启停/重启、用户中途取消与九个动作；验证 App 准入不能代替危险动作审批，自动化与子任务不可继承授权 |
| CU-R8 · M0/M3：留存与真实删除 | **仅部分证明**。现有探针通过 RPC 注入用户文字和内联 PNG，再验证归档保留及删除后历史文件内容消失、任务不可读；未覆盖真实 MCP AX/image、blob、索引、导出与备份 | 由真实 MCP 返回 AX/截图，逐项定位留存副本并验证任务删除/清理历史；覆盖子任务、共享引用、失败与重启；明确导出/备份的删除边界。发现内核缺口先按 K1 记录扩展点分析，不绕过 API 直接删生产历史 |
| CU-R9 · M3/M4：留存内容 UI | **主要界面已实现，待真实历史验收**。`cua_repl` Item 不再显示输入参数；读取项标明内容随任务保存，用户二次展开后才显示 AX/截图；删除提示区分归档、本机任务历史、项目文件与已导出/备份/模型提供方副本。模拟的迟到历史/Item 测试通过，真实 MCP 历史仍待验 | 用真实 Computer Use 历史验证默认隐藏、主动查看 AX/截图、归档保留和删除后迟到响应不重建内容；所有删除入口保持相同边界文案 |
| CU-R10 · M4：能力状态与权限 UX | **部分实现**。设置开关、App 撤权、同意选项与停止入口已有；组合插件入口、完整权限/企业/模型状态、动作计数与权限恢复引导未收口 | 真窗口验证缺组件、缺 TCC、撤权、模型不支持、企业禁用及停止路径；系统权限/组件连接/App 授权分别可见；审批三种范围与撤权即时生效 |
| CU-R11 · M0/M4：签名打包与发布 | **原生 `.app` 装配通过，发布未验收**。arm64 Helper 可执行文件约 211 KiB，`Info.plist` 校验通过；随桌面安装包的签名、公证、安装升级/TCC 与整体打包预算仍未验证 | 正式签名安装包在干净机器与升级机器上验证；TCC 首次授权、升级保留、撤权恢复通过；核对随包资源与预算；完成 §12 六个 MVP 任务和平台矩阵，保留证据 |

**发布闸门**：上述发布阻塞项关闭前，生成的 `releaseVerified=false` 保持不变；桌面不允许启用，也不提供可用 Computer Use 工具。不能通过手改标记把“未验证”变成“就绪”。Windows UIA 与 Linux Wayland/X11 驱动属于后续独立工作，不计入 macOS 首版完成条件。

**已有验证的边界**：完整检查 1696 项通过、2 项运行时安装 e2e 条件跳过；之后补充的授权 UI/宿主测试 26 项通过，桌面类型检查、Linux 构建、许可检查通过，内核补丁为 0。这些结果不替代原生、正式内核端到端、真实 MCP 存储删除及签名安装包验收。
