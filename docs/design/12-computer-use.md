...
| --- | --- | --- |
| CU-R1 · M0/M1：原生编译 | **原生 release 编译已通过，测试待补**。2026-09-24 在 macOS 27.0 / CLT 27.0、Swift `6.4.0.34.1` 与 macOS 27.0 SDK 上成功执行 `swift build -c release` 并装配 arm64 Helper `.app`；先前的 SDK 不匹配提示由沙箱内模块缓存写入受限触发，并非实际版本阻塞。修复了窗口 bounds 的 CoreFoundation 条件转换编译错误 | 补原生测试；验证 AX-only 与窗口截图的真实读状态链路，并在目标 macOS 14.4+ 版本复核 |
