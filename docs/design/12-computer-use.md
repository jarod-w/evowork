...
| --- | --- | --- |
| CU-R1 · M0/M1：原生编译 | **源码待验证**。Swift Helper 已有 AX、窗口截图和部分动作源码；Linux 环境无 Swift/macOS SDK，尚无原生编译与测试结果 | macOS 14.4+ 编译通过；补原生测试；验证 AX-only 与窗口截图的真实读状态链路，记录系统版本与构建版本 |
...
| CU-R7 · M3：真实内核与审批闭环 | **接线已有，端到端未验**。stdio MCP、宿主同意卡与内核 elicitation 适配已实现；引导技能未实现；三档动作审批/自动审查对真实 Computer Use 调用尚未验收 | 正式内核验证握手、可信调用元数据、工具发现/启停/重启、用户中途取消与九个动作；补技能；验证 App 准入不能代替危险动作审批，自动化与子任务不可继承授权 |
...
| CU-R9 · M3/M4：留存内容 UI | **待实现**。当前通用 MCP Item 展开后仍显示参数 JSON，尚无 Computer Use 专用内容查看与留存标记 | 默认隐藏输入正文；读取项标明“内容已保存到此任务”；用户主动查看 AX/截图；删除提示准确区分归档、本机任务历史、项目文件和外部副本，验证删除后迟到响应不重建内容 |
