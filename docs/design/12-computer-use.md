...
| --- | --- | --- |
| CU-R1 · M0/M1：原生编译 | **源码待验证**。Swift Helper 已有 AX、窗口截图和部分动作源码；2026-09-24 在 macOS 27.0 / CLT 27.0 上尝试构建，因 Swift 编译器 `6.4.0.34.1` 与 SDK `6.4.0.31.4` 不匹配，未进入源码类型检查；仅 `swiftc -parse` 通过 | 使用匹配的 Xcode/SDK 在 macOS 14.4+ 编译通过；补原生测试；验证 AX-only 与窗口截图的真实读状态链路，记录系统版本与构建版本 |
...
| CU-R3 · M2：完整动作面 | **待实现**。当前只开放元素单次左键点击；坐标点击、多次/其它鼠标按钮点击、滚动、拖拽与 `select_text` 的 extend 模式未完成。工具 schema 不能作为动作已实现的凭据 | 九个写动作全部实现并通过原生测试；坐标回退严格绑定截图/窗口，元素、窗口移动与过期状态均拒绝；文本输入不偷换为按键执行 |
...
| CU-R11 · M0/M4：签名打包与发布 | **构建入口已有，未验收**。Linux 构建只证明桌面与 MCP bundle；macOS Helper 编译、随包签名、公证、安装升级/TCC 和体积预算未验证 | 正式签名安装包在干净机器与升级机器上验证；TCC 首次授权、升级保留、撤权恢复通过；核对随包资源与预算；完成 §12 六个 MVP 任务和平台矩阵，保留证据 |
...
**本次续做验证（2026-09-24）**：格式、lint、全仓类型检查、1703 项测试与补丁预算通过；2 项联网安装 e2e 条件跳过。socket 测试在允许本机 Unix socket 的环境运行；技能结构校验通过。原生 `swiftc -parse` 通过，但 Swift 编译器与 SDK 版本不匹配，未得到原生编译或真机测试结果。
