...
**模型响应的解析放在 runtime，不放在内核。** 内核只吃已经结构化的 `plan.step` 事件。理由：解析要容忍模型输出的各种形态，是最容易引入非确定性（正则、时间、随机重试）的地方，把它关在内核外面，内核的确定性就好守得多。代价是 `plan.step` 的 schema 要足够表达；这个代价可以接受。**Q-12**
...
**这一条应当在 M1 内就进 CI**，不能等到有真实用例才做——M1 期先用合成 Log 也要跑起来。**Q-13**
...
快照策略建议：每 50 个事件一个，外加 `pre_write` / `pre_approval` 两个语义点。快照全部可删除——CI 里应该有一条「删掉所有快照后回放结果不变」的测试，否则半年后一定会有人往快照里塞 Log 里没有的状态。**Q-06**
...
| # | 问题 | 谁定 | 建议 |
|:-:|---|:---:|---|
| Q-12 | 模型输出解析放内核还是 runtime | 团队 | runtime。内核只吃结构化 `plan.step` |
| Q-13 | 回放自校验进 CI 的时点 | 团队 | M1 内，先用合成 Log 也要跑起来 |
| Q-06 | 快照频率、「删掉快照结果不变」是否作为硬测试 | 团队 | 每 50 事件；是 |
