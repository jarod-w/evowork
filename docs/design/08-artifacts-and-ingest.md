...
#### 信号 ① 的副产物：给任务起名（2026-09-13）

`mark_artifact --title "Q3 经营分析"` 里的显示名是**人能读的**，而任务标题此前只能从
第一条消息截（04 §3.5）—— 用的是"用户要什么"，而好认的是"它产出了什么"。

这个显示名的质量就是模型起名的质量，**因为它确实是模型写的** —— 只不过是在用户
已经付过钱的那次生成里顺手写的。所以"调模型起名"的三条代价（一次往返的钱、延迟、
一条把用户正文发出去的理由）在这里全是零：**没有新增调用，也没有新增出网路径**，K6 不受影响。

四道闸门，每一道都在挡一种比"不改名"更糟的结果（实现 `services/artifacts/src/task-title.ts`）：

1. **只认信号 ①**。②③ 的 `title` 是拿 basename 兜的底，`output.docx` 比用户原话差得多。
2. **只认 `create`**。`edit` 说明任务在改一个已存在的文件，那个名字是它自己的。
3. **只认第一版**。v2 是同一份产物重渲染，主题没变。
4. **显示名不能就是文件名**。技能没传 `--title` 时等同第 1 条没满足。

再叠一条**第一个产物赢**（靠 `thread_projection.title_source`，09 §4.1）：一个任务产出
三个文件时，标题在侧边栏里跳三次比一个平庸的标题更糟。用户改过名之后产物不再碰它。
...
- 若当前走内部只读路径（Q20 助理，`evowork-ask`），`uploads/` 的写入由**服务层**完成（不是 agent），因此不违反只读语义 —— 但要在 UI 上说明"上传的文件会保存到工作空间"。Composer 三档（Q45）都可以写工作空间，不走这条例外。
...
产物的生成或发现不会机械地打开右侧工作区。用户点击产物卡时打开；只有任务意图明确要求持续预览、比较或交互时，系统才可主动打开。用户关闭后，本任务内不得再次自动打开。

| 位置               | 形态                                                                    | 文档    |
| ------------------ | ----------------------------------------------------------------------- | ------- |
| 对话流内           | 生成完成后插入产物卡（文件名 + 类型 Badge + 预览缩略 + 打开/另存；分享接通后再显示） | 04 §5   |
...
> **当前实现边界（2026-09-19）**：任务切换时从权威产物索引加载结果，
> 对话流显示轻量产物卡并可打开结果区。正文里与产物同名的行内文件名也可点开。
> 技能上报与 FileChange 写到任务 cwd 之外（如桌面）时仍归当前任务。
> 文本/代码、图片、PDF 和 HTML 使用本机只读预览；
...
**当前实现状态**：桌面宿主调用方与云端上传/托管端点尚未接通，因此任务、产物和资料库中的分享入口必须隐藏。以下内容是接通后的验收规格，不是当前可用能力。
...
### 7.4 云端分享页怎么呈现（Q41，2026-09-08）

上游决策在 [11 §13.9](11-account-and-models.md)。本小节只写产物这一面的细则。

链接接收方打开 `/s/<share-id>`（浏览器，**不需要账号**）。页面与管理端、账号 cookie 隔离。

| 产物类型 | 呈现 | 为什么 |
| --- | --- | --- |
| **docx / xlsx / pptx** | 文件名 · 类型 Badge · 大小 · 有效期 · 下载 · 「在 EvoWork 中打开」 | **不在浏览器里渲染、不执行宏、不转 HTML**。办公文件当网页跑，和 04 Visualizer 同时给 `allow-scripts` + `allow-same-origin` 是同一类破口 |
| 图片 / PDF | 同上 + 预览 | 预览 CSP 收死；PDF 用浏览器原生 viewer 或沙箱 iframe，**不给 `allow-same-origin`** |
| 已过期 / 已撤销 | 失效页 | 不继续提供下载；文件名以外的元数据能少则少 |
| 设了访问密码（§7.1） | 先过密码再给下载 | 密码只挡住获取文件，**不因此开通办公文件预览** |

「在 EvoWork 中打开」走 `evowork://share/<shareId>`（02 §8）。本机没有这份产物时如实说「该文件不在这台电脑上」，不假装同步过来（D6 / Q17）。

可选「保存到我的资料」= **一次显式下载**到本机资料目录（06 §4.3），不是云端再存一份。
