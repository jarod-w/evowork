 ...
 * | 组合 | 实测 | 后果 |
 * |---|---|---|
 * | `--text-secondary` on `--bg-app` | **9.70** | 余量充足；仍不得叠加透明度跌破 4.5 |
 * | `--text-tertiary` on `--bg-app` | 3.07 | 不满足正文要求 → 只能用于时间戳/占位符/版本号 |
 * | `--border-default` on `--bg-surface` | 1.30 | 目标 3.0 不达标 → 这是**刻意的取舍**（§8.3），靠高对比模式兜 |
 * | `--success` on `--success-weak` | 4.18 | 语义色文字必须用 `-text` / `-strong` 变体 |
 *
