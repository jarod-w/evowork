...
#[derive(
    Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Default, Serialize, Deserialize, TS,
)]
...
impl TaintLevel {
    ...
    pub fn join(self, other: Self) -> Self {
        if self == Self::Tainted || other == Self::Tainted {
            Self::Tainted
        } else {
            Self::Clean
        }
...
mod tests {
    ...
    fn taint_only_goes_up() {
        ...
        assert_eq!(
            TaintLevel::Clean.join(TaintLevel::Tainted),
            TaintLevel::Tainted
        );
        assert_eq!(
            TaintLevel::Tainted.join(TaintLevel::Clean),
            TaintLevel::Tainted
        );
    }
