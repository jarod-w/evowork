...
`node scripts/build-computer-use.mjs` 在 macOS 构建 Swift 并装配 `build/computer-use/EvoWork Computer Use.app`。2026-09-24 在 macOS 27.0 / CLT 27.0 上尝试编译，Swift 编译器 `6.4.0.34.1` 与 SDK `6.4.0.31.4` 不匹配；`swiftc -parse` 通过，但尚无类型检查、原生编译或真机验证结果。构建出的发布标记强制 `releaseVerified=false`，桌面不能启用，也不会注册可用工具。
