 ...
 *
 * 单独一个文件、导出成普通函数，是为了能直接测「用户选过的模型不可用时会发生什么」——
...
export interface ModelChoice {
   ...
   * 需要显示给用户的话。
   *
   * 只有一种情况非空：**用户自己选过的模型当前不可用，我们换了一个**。
   * 这句话不能省 —— 静默换模型正是 03 §8 与 D2 都禁止的那件事：
   * 用户以为在用 A、实际在用 B，而账单与产物质量都不一样（U1 就是关于产物质量的）。
   *
   * 场景包里的 `model` 不在此列：那是偏好，目录里没有它时，这台机器上第一个
   * 可用的模型**就是**默认（Q30 未登录只有自定义模型）。再弹「场景默认不可用」
   * 是在陈述一件用户从未配置过的事。
   */
...
/**
 * 选哪个模型：**优先用户已选的，其次场景偏好（目录里有才算），最后第一个可用的**。
 *
 * `chosen` 是用户显式选过的那个（03 §2.5 的"已被你改过"）。它在列表里就一直生效，
 * 切场景也不改 —— 场景偏好只在用户没选过时才有发言权。
 *
...
export function resolveModelChoice(
  ...
  const preferred =
    scenarioDefault && models.some((m) => m.id === scenarioDefault) ? scenarioDefault : first.id;
  const preferredModel = models.find((m) => m.id === preferred) ?? first;

  if (chosen) {
    return {
      modelId: preferred,
      notice: `你选的模型「${chosen}」当前不可用（网关没有它，或那家厂商的密钥没配），已改用「${preferredModel.label}」。`,
    };
  }
