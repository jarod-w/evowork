...
| `prompts/base-instructions.md` | EvoWork      | 产品身份底稿。内核 `default.md` 的 fork，只改身份段；经 `thread/start.baseInstructions` 整段替换（F25） |
| `modes/{craft,plan,ask}.md`    | EvoWork      | developer instructions 片段。**取代了原 P3 补丁**（F1）；管怎么干活，不管产品名                         |
